 ...
 * self-heals if reached any other way while the flag is off or the driver's
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // driver on a screen that failed to load, same posture as any other
        // best-effort check in this codebase (a monitoring/gating check
        // must never block the primary flow it's attached to).
        if (!cancelled) {
          router.replace('/driver' as any);
          return;
        }
