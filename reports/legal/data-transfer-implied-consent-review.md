...
| If disclosure is required: add language to `docs/legal/privacy-policy.md` before publication | Legal | ⬜ Open — tracked in `ACTION_ITEMS.md` B11 and `docs/legal/legal-text-publication-checklist.md`'s `privacy-policy.md` row. **Note:** the privacy policy is already live in production (published 2026-08-17, ahead of its own required counsel review) — this disclosure sentence should be added at its next reviewed/re-published version, not as a standalone unreviewed edit to the already-live text. | — |
