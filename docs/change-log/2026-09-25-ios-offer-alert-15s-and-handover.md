...
3. **Confirmed in library source, NOT reproduced on a device.** expo-audio Android (`AudioModule.kt` `OnActivityEntersBackground`/`Foreground`) marks any playing player `isPaused = true` on background and replays every `isPaused` player on return. The JS `pause()` in `useRideOfferSound.stop()` never clears that flag, so a tone caught mid-clip resumes on return even if its offer expired meanwhile (`ride_offer.mp3` ≈ 1.9 s ≈ the reported 1–2 s). An earlier hypothesis (a late `play()` after an async handover) was investigated and **dropped** — an independent review showed it does not explain the symptom, and the guard for it was reverted before this log.

## 3. Fix / remediation

1. `ride_offer.caf` regenerated: exactly 15.00 s (the offer window; the file is the only way to "repeat" on iOS), peak 0.98, RMS 0.206 (≈ +6 dB average, ≈ +2 dB peak).
...
3. `useRideOfferSound.stop()` on **Android, when the app is not in the foreground**, now releases the player (`pause()` + `remove()`), removing it from expo-audio's native resume list. `_getOrCreatePlayer()` rebuilds it on the next offer — from the admin-uploaded URL when one is set, not the bundled tone.
...
| `driver-app/hooks/useDriverDashboard.ts` | lazy binding, foreground-gated helper, 3 call sites | iOS handover |
...
- [x] Automated: 10 suites / 141 tests pass (`notifeeService`, `useRideOfferSound.release`, `useRideOfferSound.carOwner`, `useDriverDashboard.socketLifecycle`/`.chat`/`.alwaysLocationGate`/`.wsSenders`, `backgroundMessaging` iOS+Android, `pendingRideOffer`).
