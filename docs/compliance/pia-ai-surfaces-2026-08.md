...
| `tools_booking.py` (`find_place`, `get_rider_location`, `get_fare_quote`, `propose_ride_booking`) | **Precise pickup/dropoff GPS coordinates**, resolved addresses, device location fix, most-recent-ride pickup as a location hint, fare totals, promo application | Location (precise GPS), Financial | Partially (amended 2026-09-04, ADR-012): the model-facing result is scrubbed in `tools.py::_cap_result` under `ScrubPolicy.AI_CHAT` — identifiers redacted; Canadian postal codes and app-generated bracketed `[lat,lng]` pins kept; numeric coordinates survive as non-string JSON leaves (value-only scrub); free-text coordinate strings are still redacted. The rider-facing `_client_action` card is not scrubbed at all (it returns to the data subject) |
...
| User's own chat message | Free text, scrubbed for phone/email/free-text GPS/card number/grouped SIN; bracketed trip-pin coordinates **and, since 2026-09-04, Canadian postal codes** deliberately preserved (`ScrubPolicy.AI_CHAT`, ADR-012 — the street address beside them was never scrubbable) | Any (self-reported) | **Yes**, partially (see Section 6 for gaps) |
...
|---|---|---|
| User's chat message → persisted to `ai_messages`, and thus replayed to the LLM provider on the next turn | **Yes** (`policy=ScrubPolicy.AI_CHAT` since 2026-09-04; was `keep_trip_pins=True`) | `orchestrator.py:196` |
...
| Assistant's final reply → persisted to `ai_messages` / stored to response cache | **Yes** (`policy=ScrubPolicy.AI_CHAT` since 2026-09-04; was `keep_trip_pins=True`) | `orchestrator.py:393` |
...
| **Tool-result JSON appended to `messages` mid-turn** (`{"role": "tool_result", "content": json.dumps(result, ...)}`) | **No — never scrubbed at all**, in any adapter, for any provider. **Amended 2026-08-18 / 2026-09-04:** now scrubbed upstream in `tools.py::_cap_result` under `ScrubPolicy.AI_CHAT` (identifiers redacted; postal codes and bracketed trip pins kept; numeric coordinates survive as non-string leaves; free-text coordinate strings still redacted); the anonymous `web` audience stays `STRICT` (`tools._policy_for_audience`); the `_client_action` card is exempt because it goes to the rider's own app, not the provider (ADR-012) | `orchestrator.py:356-364` |
| `find_place`/`get_fare_quote`/`propose_ride_booking` results (GPS, addresses) | Not scrubbed — and the one `scrub_pii` call in the booking-tool path (none exists) is irrelevant here since these results flow through the tool-result path above, which is unscrubbed by construction. **Amended 2026-08-18 / 2026-09-04:** see the tool-result row directly above — these results are scrubbed in `tools.py::_cap_result` under `ScrubPolicy.AI_CHAT`; the card is exempt (ADR-012) | `tools_booking.py` |
...
| MCP tool results → external agent client | **No** — `mcp_server.py`'s `_call_tool` calls `execute_tool` directly and returns the raw result; same unscrubbed tool-result path. **Amended 2026-09-04:** `_call_tool` now serializes through `mcp_server._serialize_tool_payload`, which re-scrubs the whole response (card included) under `ScrubPolicy.STRICT` (ADR-012) | `mcp_server.py:163-176` |
| Escalation transcript → Zoho ticket | **Yes**, inherits scrub because it's built from already-persisted (scrubbed) `ai_messages` rows via `conversations.load_history`. **Amended 2026-09-04:** persistence now keeps postal codes and bracketed trip pins (`ScrubPolicy.AI_CHAT`), so `_recent_transcript` re-scrubs every line under `STRICT` at the Zoho boundary before truncation (ADR-012, review round 1) | `tools_support.py::_recent_transcript` |
...
This assessment is a point-in-time record (2026-08-02). Later code changes that alter what reaches a
provider are logged here rather than by rewriting the sections above; the affected rows carry an
"Amended" marker pointing back to this table.

| Date | Change | Net effect on provider exposure | Record |
|---|---|---|---|
| 2026-08-18 | Tool-result JSON is PII-scrubbed at `tools.py::_cap_result` for the regex-detectable categories (phone, email, GPS, postal, card, grouped SIN). Closes the "never scrubbed at all" rows in Section 8 for those categories only; names and government IDs remain a data-minimisation matter. | Reduced | `docs/change-log/2026-08-18-ai-tool-result-pii-scrub-fix.md`; `ACTION_ITEMS.md` A40 |
| 2026-09-04 | `ScrubPolicy` replaces `keep_trip_pins`. On the authenticated in-app chat path (`ScrubPolicy.AI_CHAT`: user message, persisted reply, model-facing tool results) Canadian postal codes are now carried to the provider alongside the street address and coordinates that already were. The rider-facing `_client_action` card is never scrubbed (it returns to the data subject). `/mcp` re-scrubs its whole response under `STRICT`. Logs, Sentry, `routes/support.py` tickets and the anonymous web assistant's message text are unchanged; the anonymous web assistant's tool results stay `STRICT` (`tools._policy_for_audience`); the AI escalation transcript to Zoho is now explicitly re-scrubbed `STRICT` at its boundary because persistence no longer is. Second-order: the cross-user FAQ response cache stores the persisted (`AI_CHAT`) reply, so a first-turn reply that echoes a postal code the visitor typed can be cached — keyed by the exact question hash and limited to first-turn, tool-free FAQ turns. | Marginal increase: a postal code (block-face granularity) beside an already-transmitted street address (building granularity). Accepted under the Section 3 trip-endpoint exception. | ADR-012; `docs/change-log/2026-09-04-ai-chat-postal-code-card-scrub.md`; `ACTION_ITEMS.md` AI16 |
