...
const CarMarkerComponent: React.FC<CarMarkerProps> = ({
    ...
    const ingestFix = useCallback((fix: MarkerFix) => {
        ...
        if (isFirstFix || shouldResetBuffer(bufferRef.current, rawCoord, SNAP_DISTANCE_M)) {
            ...
            if (isFirstFix) {
                ...
                // (a ride-phase screen remount, a real background/tunnel
                // gap) — live-testing report (driver-app parity):
                // the marker snapping to a wrong heading (e.g. "facing
                // east") right after one of these resets, before the next
                // real movement re-established a trustworthy bearing.
    ...
    useEffect(() => {
        const id = setInterval(() => {
            ...
            // there — and on driver-app that feeds the course-up camera, so
            // the whole map would rock back and forth. This is exactly
            // coalescePlaybackBearing's own condition; 'extrapolating' needs no
            // such guard because that branch always carries a computed bearing
            // (a null one falls through to 'holding' instead).
