...
| R11 | As a rider, I can add stops mid-trip | ✅ | ✅ | — | `test_p1_multi_stop.py` + `rideStore.stops.test.ts`; fare recalc now pinned |
...
9. ✅ **Multi-stop E2E** — R11 — closed: `backend/tests/test_p1_multi_stop.py` pins add/remove stop mid-trip (WS driver notification, auth guards, position insertion, bad-index rejection); `rider-app/store/__tests__/rideStore.stops.test.ts` pins pre-ride addStop/removeStop/updateStop; **fare recalc now implemented** via `_recalculate_fare_for_stops()` — scales distance_fare/time_fare by the new/old distance ratio so the rider's per-km rate (with surge already locked at create-time) is preserved; the new fare ships on the `stops_updated` WS event so the driver sees the revised total without a fetch
