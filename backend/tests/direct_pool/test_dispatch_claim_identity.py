"""High-risk SQL regressions for migration 444's durable claim identity.
...
in Python. They become runnable once migration 444 is included with the
...
def test_stale_reaper_fails_closed_on_null_p2_identity(pg_cur, period_ride, period_claim, expected_status):
    ...
    pg_cur.execute('SELECT is_available, availability_claim_id FROM drivers WHERE id = %s', ('identity-reap-null-driver',))
    assert pg_cur.fetchone() == (False, claim_id)
