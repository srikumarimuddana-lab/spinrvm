...
| `_driver_roster_rows` | Only `get_driver_roster` (both download and email variants) | Behaviour change to an outgoing report — see UX below. |
...
| Migration 274 columns | New; only the three call sites added here | Purely additive. |
...
- **Migration 274 backfills existing soft-deleted, regulator-approved drivers into the queue.** Intentional: those are exactly the rows SGI still lists, and starting the queue empty would leave the real backlog invisible. It is an `UPDATE` over a small set (soft-deleted drivers only) guarded by `regulator_removal_required = FALSE`, so it is re-runnable and cannot double-apply.
...
| `backend/migrations/274_drivers_regulator_removal_queue.sql` | Removal-queue columns + partial index + backfill | Storage for the queue and its audit trail |
...
- **Migration 274**: the down-migration is in the file's top comment, per `backend/migrations/CLAUDE.md`:
  ```sql
...
- [x] **Migration apply simulated, not assumed** — reimplemented `scripts/migrate.py::_apply_migration_autocommit`'s splitter and ran migration 274 through it. **This caught a defect the review did not**: because the file contains `CONCURRENTLY`, the runner splits it on semicolons, and two *mid-line* semicolons inside prose comments were producing bogus statements (`a plain build blocks every write to it…`) that Postgres would have rejected — after the `ALTER TABLE` had already committed under autocommit. Comments reworded; the simulation now yields exactly 8 clean statements. Pinned by `tests/test_migration_concurrently_splitting.py`.
...
- **Migration 274 has not been applied anywhere.** The backfill's row count in production is unknown; it should be previewed with `python scripts/migrate.py --dry-run` and the `UPDATE`'s effect sanity-checked against the real count of soft-deleted approved drivers before applying.
