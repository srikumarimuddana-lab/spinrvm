...
try:
    from ...utils.ride_state_copy import driver_phrase, unavailable_message
except ImportError:  # pragma: no cover - direct-module execution path
    from utils.ride_state_copy import driver_phrase, unavailable_message  # type: ignore
...
# Pickup OTP is generated by dependencies.generate_pickup_otp() as
# PICKUP_OTP_LENGTH (4) digits. The upper bound is deliberately looser than the
# generator so lengthening the code later does not need a coordinated
# client+server release; the digit-only pattern is the part that matters.
#
# Without these constraints the field accepted any string, so a submitted "" or
# a non-digit blob reached hmac.compare_digest() and either matched a NULL/blank
# stored OTP or raised TypeError -> 500 (audit 2026-09-05 §1.6).
_PICKUP_OTP_MIN_LEN = 4
_PICKUP_OTP_MAX_LEN = 6
...
# ── Pickup-OTP brute-force lockout ────────────────────────────────────────────
# The pickup OTP is the only production gate between `driver_arrived` and
# `in_progress` — i.e. between Period 2 and Period 3 commercial insurance and a
# running meter. A 4-digit code is 10_000 possibilities, and the only limiter
# used to be the global 100/minute-per-IP default, which lets the *assigned*
# driver exhaust the space in well under an hour with no rider aboard.
#
# The counter is keyed per ride, not per driver: a ride is a one-shot object
# with its own code, so 5 attempts per ride makes targeted brute force
# infeasible while a driver who fat-fingers one code is unaffected on the next
# ride. Mirrors the admin TOTP lockout shape in routes/admin/auth.py.
_PICKUP_OTP_MAX_FAILURES = 5
# 15 minutes, matching _TOTP_LOCKOUT_TTL_SECONDS. Deliberately NOT the 24h
# phone-OTP lockout: a locked pickup OTP blocks a real trip that a real rider is
# sitting in, and support/the rider can resolve it by cancelling and rebooking.
_PICKUP_OTP_LOCKOUT_TTL_SECONDS = 15 * 60
# Counter window. Longer than the lockout so attempts don't silently reset
# under a sustained slow-drip attack.
_PICKUP_OTP_FAILURE_WINDOW_SECONDS = 60 * 60


def _pickup_otp_fail_key(ride_id: str) -> str:
    return f"spinr:ride:{ride_id}:otp_fail"


def _pickup_otp_lock_key(ride_id: str) -> str:
    return f"spinr:ride:{ride_id}:otp_lock"


def pickup_otp_locked_exc() -> HTTPException:
    """The 429 raised while a ride's pickup OTP is locked.

    Response shape mirrors utils/rate_limiter.py's 429 so mobile clients parse
    one format regardless of which gate tripped (RateLimit-* per
    draft-ietf-httpapi-ratelimit-headers, Retry-After per RFC 9110).
    """
    return HTTPException(
        status_code=429,
        detail="Too many incorrect pickup codes for this ride — try again later",
        headers={
            "Retry-After": str(_PICKUP_OTP_LOCKOUT_TTL_SECONDS),
            "RateLimit-Limit": str(_PICKUP_OTP_MAX_FAILURES),
            "RateLimit-Remaining": "0",
            "RateLimit-Reset": str(_PICKUP_OTP_LOCKOUT_TTL_SECONDS),
        },
    )
...
async def record_pickup_otp_failure(ride_id: str, driver_id: str) -> int:
    """Increment this ride's failure counter; lock at the threshold.

    Best-effort (mirrors _record_otp_failure): a Redis failure here must not
    convert a wrong-code 400 into a 500. Returns the failure count, or 0 when
    ...
    try:
        ...
        if count >= _PICKUP_OTP_MAX_FAILURES:
            ...
            _deps._metric_inc("spinr_rides_pickup_otp_lockout_total")
        return int(count)
    except Exception as e:
        logger.error(f"record_pickup_otp_failure ride={ride_id}: {e}", exc_info=True)
        return 0
...
async def clear_pickup_otp_failures(ride_id: str) -> None:
    ...
    try:
        ...
        logger.error(f"clear_pickup_otp_failures ride={ride_id}: {e}", exc_info=True)
...
async def _require_ride_in_state(ride_id: str, driver_id: str, allowed_states: tuple) -> Dict[str, Any]:
    ...
    if existing:
        ...
        raise HTTPException(
            status_code=409,
            detail=unavailable_message(driver_phrase(current)),
        )
