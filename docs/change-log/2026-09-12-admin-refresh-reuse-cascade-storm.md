...
| **Fix/remediation** | Backend `utils/refresh_tokens.py`: (a) new `REFRESH_REVOKE_RACE_GRACE_SECONDS = 60` — a non-rotated revoke replayed within 60 s is a client race: generic 401, no cascade, **admin audience only** (the mobile clients await their logout before navigating, and rider/driver credential theft is the threat model the cascade exists for), and the replay is still **recorded** — an `audit_logs` row with `cascade_ok: false` (so it never suppresses a later real cascade) plus a Sentry warning tagged `spinr_alert=refresh_token_post_revoke_replay` — because the revoke may have been an admin force-logout on a suspected account (rotated revokes keep the 600 s log-only grace unchanged); (b) the cascade records `cascade_revoked_row_ids` in its `audit_logs.details` (new `revoke_all_for_user_ids`; `revoke_all_for_user` still returns the count) and `_reuse_already_handled` recognises those ids, so a victim's replay is logged/alerted as a repeat, not cascaded. Dashboard: `logout()` returns the BFF call's promise (never rejects); `lib/api/client.ts` awaits it before the hard redirect so the cookie is gone before `/login` bootstraps. |
...
| **Verification performed** | Backend: `pytest` on the six files touching these functions — **201 passed** (`test_refresh_token_reuse_detection.py` 31, incl. the new ones; `test_logout_all.py`, `test_refresh_tokens_lifecycle.py`, `test_admin_staff_coverage.py`, `test_admin_staff_mfa_reset.py`, `test_routes_users_coverage.py`); `ruff check` + `ruff format --check` clean. Dashboard: `vitest` on the auth store, BFF refresh route, CSRF-retry and the two new files — **33 passed** (incl. `auth-bootstrap-contract.test.ts`, which pins the root-layout-only, once-per-page-load bootstrap that keeps `topbar.tsx`'s unawaited `logout(); router.push` safe); `tsc --noEmit` clean; `eslint` 0 errors (1 pre-existing warning on the `window.location.href` line); `npm run build` — see PR. `spinr-security-auditor` pass on the diff — see PR. |
| **What was NOT verified** | Not reproduced against a live backend; the 1.3 s race was reconstructed from `refresh_tokens` + `audit_logs` rows, not observed in a browser. `topbar.tsx`'s `logout(); router.push("/login")` (client-side navigation, no bootstrap) is safe by the invariant the contract test pins, not by a browser run. The admin-dashboard visual-regression job is unaffected (no page changed) — stated, not screenshotted. Mobile clients' own logout/refresh overlap is covered by the backend window but was not traced in `shared/store/authStore.ts`. |
