...
- Blast radius — **corrected after money-auditor review**. An earlier draft of this entry listed `utils/route_distance.py`, `utils/route_finalizer.py`, `utils/trip_distance.py`, `utils/route_reconstruction_projection.py` and `routes/drivers/_shared.py` as consumers. **They are not.** Those read a similarly-shaped `{"polyline": ...}` dict from an unrelated function, `compute_road_route` (`utils/route_distance.py:424`) — the GPS-breadcrumb snap used for post-trip distance reconciliation, which this diff does not touch. Matching on the dict shape rather than on the call graph produced a false consumer list; recording the mistake because it gave unearned confidence.
- The actual callers of `_fetch_directions_route` / `_fetch_directions_polyline` are three, all in the rides package:
  - `routes/rides/estimates.py:296` (pricing) — `_final_route.get("polyline") if _final_route else None`; an empty list renders as "no line", same as today's `None`.
  - `routes/rides/booking.py:1363` — `if _computed:` treats `[]` and `None` identically.
  - `routes/rides/booking.py:604` — the no-valid-token fallback; reads `distance_km` only.
  (Plus an unused re-export in `routes/rides/__init__.py`.) None regress on `polyline: []`.
- **`booking.py:604` inherits the timeout bump**, so worst-case `POST /rides` grows ~1.5 s on bookings without a valid estimate token (expired token, non-app client). Deliberately **not** given a tighter `asyncio.wait_for`: it is already bounded by the Directions HTTP timeout, and cutting it shorter would reinstate exactly the haversine undercharge this change exists to stop. Flagged rather than "fixed" — unlike the estimate path it has no ratchet test, so a future timeout change must remember it.
