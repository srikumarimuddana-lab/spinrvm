...
class CloudinaryService:
    ...
    def __init__(self, cloud_name: str = "", api_key: str = "", api_secret: str = "", secure: bool = True):
        ...
        if self.configured:
            cloudinary.config(cloud_name=cloud_name, api_key=api_key, api_secret=api_secret, secure=secure)
    ...
    async def upload_image(
        ...
        try:
            upload_options = {"folder": folder, "tags": tags or [], "resource_type": "image"}
    ...
    async def upload_from_url(
        ...
        if not self.configured:
            ...
            return {"success": True, "secure_url": image_url, "public_id": public_id or "mock_image"}

        try:
            result = cloudinary.uploader.upload(
                image_url, folder=folder, public_id=public_id, tags=tags or [], resource_type="image"
            )
    ...
    def get_image_url(
        ...
        return cloudinary.utils.cloudinary_url(public_id, transformation=transformation_str)[0]
    ...
    async def delete_image(self, public_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "result": result.get("result", "ok")}
        except Exception as e:
            logger.error(f"Failed to delete image from Cloudinary: {e}")
            return {"success": False, "error": str(e)}

    async def list_images(self, folder: str = "spinr", max_results: int = 100) -> Dict[str, Any]:
        """
        ...
        try:
            result = cloudinary.api.resources(type="upload", prefix=folder + "/", max_results=max_results)
...
def init_cloudinary(cloud_name: str, api_key: str, api_secret: str) -> CloudinaryService:
    """
    ...
    """
    global _cloudinary_service
    _cloudinary_service = CloudinaryService(cloud_name=cloud_name, api_key=api_key, api_secret=api_secret)
    return _cloudinary_service
