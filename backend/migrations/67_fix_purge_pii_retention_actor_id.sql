-- 67_fix_purge_pii_retention_actor_id.sql
-- Rollback: replace actor_id with user_email in the INSERT below (migration 51
--           already dropped the user_email column so true rollback requires
--           re-adding it first).
--
-- Migration 51 (51_audit_logs_remove_user_email.sql) dropped the user_email
-- column from audit_logs. Migration 57_purge_pii_retention_regression_fix.sql
-- redefined purge_pii_retention() but still referenced user_email in its
-- audit INSERT, causing every retention-purge tick to fail with PGRST204.
-- This migration replaces user_email with actor_id (added by migration 57
-- _audit_logs_schema_standardization.sql) in the function body.
...
COMMENT ON FUNCTION purge_pii_retention(BOOLEAN) IS
    'B-P1-6 + B-P1-7 + B-P2-4 + DV-8 retention enforcement. Uses actor_id (migration 57 schema). p_dry_run=true returns counts without mutating.';
