...
export default function DisputesTab() {
    ...
    const [form, setForm] = useState({ ride_id: "", user_name: "", user_type: "rider", reason: "fare", description: "", refund_amount: "" });

    const load = async () => {
        setLoading(true);
        try {
            const data = await getDisputes();
            setDisputes((data || []).map((d: any) => ({ id: d.id, ride_id: d.ride_id, user_name: d.user_name || "Unknown", user_type: d.user_type || "rider", dispute_type: d.reason || "other", description: d.description, status: d.status || "pending", refund_amount: d.refund_amount, created_at: d.created_at, resolution: d.admin_note || d.resolution_notes || d.resolution })));
    ...
    const filtered = disputes.filter((d) => {
        ...
        return ms && (statusFilter === "all" || d.status === statusFilter || (statusFilter === "pending" && d.status === "open"));
    });

    const reset = () => { setForm({ ride_id: "", user_name: "", user_type: "rider", reason: "fare", description: "", refund_amount: "" }); setEditing(null); };

    const handleSave = async () => {
        if (!form.description.trim()) { alert("Enter a description."); return; }
        setSaving(true);
        try {
            if (editing) await updateDispute(editing.id, { reason: form.reason, description: form.description, refund_amount: form.refund_amount ? parseFloat(form.refund_amount) : 0, user_type: form.user_type });
            else await createDispute({ ride_id: form.ride_id || null, user_name: form.user_name, user_type: form.user_type, reason: form.reason, description: form.description, refund_amount: form.refund_amount ? parseFloat(form.refund_amount) : 0 });
    ...
    return (
        <div className="space-y-4">
            ...
            <Card><CardContent className="p-0">
                ...
                : <Table><TableHeader><TableRow><TableHead>User</TableHead><TableHead>Type</TableHead><TableHead>Description</TableHead><TableHead>Status</TableHead><TableHead>Date</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                    <TableBody>{filtered.map((d) => {
                        ...
                        return (
                            <TableRow key={d.id} className="cursor-pointer hover:bg-muted/50" onClick={() => { setSelected(d); setResolution(""); }}>
                                ...
                                <TableCell className="max-w-[180px] truncate text-xs text-muted-foreground">{d.description}</TableCell>
                                ...
                                <TableCell className="text-right"><div className="flex justify-end gap-0.5">
                                    ...
                                    <Button variant="ghost" size="icon" className="h-7 w-7" onClick={(e) => { e.stopPropagation(); setEditing(d); setForm({ ride_id: d.ride_id || "", user_name: d.user_name || "", user_type: d.user_type || "rider", reason: d.dispute_type || "fare", description: d.description || "", refund_amount: d.refund_amount ? String(d.refund_amount) : "" }); setDialogOpen(true); }}><Pencil className="h-3.5 w-3.5" /></Button>
