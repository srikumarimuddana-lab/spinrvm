...
## Risk & impact
Load-test operators must set an exact comma-separated origin allowlist, such as `LOADTEST_ALLOWED_ORIGINS=https://staging-api.spinr.ca`. The pre-auth entrypoint validates the target before creating its HTTP session. No application behavior or live data changes. The seed script's production Supabase project guard is reviewed separately.
...
- [x] `/tmp/pr5725-venv/bin/python -m pytest loadtest/test_target_guard.py loadtest/test_preauth_target_guard.py -q` — 14 passed; `git diff --check` clean.
- [x] All checks use synthetic origins and fake clients; no network target was contacted.
- [ ] Locust target/cache wiring, seed database guard, and a real staging load run are separate follow-ups; no load run was executed.
