...
- **Files:**
  ...
  - Sub-tier B batch 1 — `routes/main.py`, `utils/t4a_pdf.py`,
    ...
    - Test-only across all three; no application code changed. Blast
      ...
      unrelated concurrent session's Sub-tier B batch 2 work on
      `services/zoho_desk_db.py`/`utils/zoho_desk_sync.py`, a separate
      branch/PR): 7623 passed (baseline, this branch's files removed) →
      7659 passed (this branch's files restored), 8 skipped, 1 xfailed, 0
      failed both times — delta +36 matches the 36 new tests added here, 0
      regressions. See
