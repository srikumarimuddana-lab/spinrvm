...
-- audit_logs rows for today. No new table.
--
-- Semantics: NULL = disabled. Both columns ship NULL, so nothing changes until
-- the owner sets values (founder decision E-F7) via PUT /api/admin/settings.
--
-- Additive + idempotent (ADD COLUMN IF NOT EXISTS, nullable, no default) -- a
-- metadata-only change, no table rewrite, safe under live traffic. The backend
