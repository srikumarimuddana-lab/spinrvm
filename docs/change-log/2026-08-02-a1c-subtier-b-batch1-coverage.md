...
- [x] Coverage measured (same command as above):
  ...
  - `utils/subscription_invoice_pdf.py`: **7.97% → 99%** (137/138 stmts —
    ...
    below)
- [x] Full backend suite before this session's Sub-tier B work started:
  **7573 passed, 8 skipped, 1 xfailed** (measured via the task's own
  baseline command, `pytest tests/ -q --cov=routes.main --cov=utils.t4a_pdf
  --cov=utils.subscription_invoice_pdf --cov-report=term-missing
  --no-cov-on-fail`, 531.85s).
- [x] Full backend suite after (`pytest tests/ -q --no-cov`, this batch's
  three files only — excludes an unrelated concurrent session's
  `test_zoho_desk_db.py`/`test_zoho_desk_sync.py` that happened to be
  sitting untracked in the same working tree at run time, not part of this
  PR): **7609 passed, 8 skipped, 1 xfailed, 0 failed** (baseline was 7573
  passed, 8 skipped, 1 xfailed — delta is exactly +36, matching the 36
  tests added here (13 + 12 + 11), 0 regressions).
...
- A concurrent session's Sub-tier B batch 2 log (`services/zoho_desk_db.py`,
  ...
  calling it a pre-existing order/timing-dependent flake. This batch's own
  runs — standalone, combined with each target's pre-existing related test
  files, and two independent full-suite runs (7659 passed with the batch 2
  files also present, 7609 passed without them) — were all clean, 0
  failures. Not independently re-chased beyond that given the two clean
  full-suite runs already performed here; flagged for whoever next touches
  this file if it recurs, rather than claimed as definitively ruled out.
