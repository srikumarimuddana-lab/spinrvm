...
4. Confirmation that the production `driver_bank_import`/`driver_csv_import` staging tables have actually been dropped, per each script's own final step — not established by any entry in this register yet; needs someone with Supabase access to check and record here.
