...
**Status (2026-09-23):** First-wave Tasks 1–11 implemented, reviewed and published to PR 5725. See [execution evidence and remaining release gates](../../audit/2026-09-23-pr5725-implementation-status.md). Subsequent waves below remain open.
...
- [x] Test no URL, initialization failure, SET failure, contention, acquisition and expired ownership.
- [x] Implement direct `_get_redis()` + `r.set(key, value, nx=True, ex=ttl)`; raise a fixed unavailable error when `r is None`.
- [x] Run `python -m pytest tests/test_redis_client_coverage.py -q --no-cov` from backend, then commit `fix(redis): add fail-closed distributed lock primitive`.

### Task 2: Bounded Redis waits
Files: Redis client, its existing coverage test, `docs/change-log/2026-09-23-redis-timeouts.md`.
- [x] Assert connection and command timeouts are passed to `from_url`, with timeout retries disabled.
- [x] Configure bounded waits; preserve strict error propagation and sanitize initialization diagnostics.
- [x] Run the Redis coverage suite; commit `fix(redis): bound connection and command waits`.

### Task 3: Payment retry gate
Files: `backend/utils/payment_retry.py`, `backend/tests/test_payment_retry_coverage.py`, `docs/change-log/2026-09-23-payment-retry-lock.md`.
...
- [x] Drive the real loop through failed acquisition then healthy acquisition; assert no financial tick on failure and normal recovery/heartbeat. Retain virtual-clock cadence and claim/idempotency tests.
- [x] Change lock error to `got_lock = False`; record `spinr_loop_lock_unavailable_total{loop="payment_retry"}`.
- [x] Run payment retry and payment retry coverage suites; commit `fix(payments): skip retry ticks without Redis ownership`.
...
- [x] Test lock outage skips `_capture_tick`, contention skips, recovery resumes, and cancellation still propagates.
- [x] Import strict primitive under existing local name, fail closed and emit lock-unavailable counter; retain DB claim and existing lease duration.
- [x] Run both preauth capture suites; commit `fix(payments): fail closed on preauth lock outages`.

### Task 5: Orphaned hold gate
Files: `backend/utils/orphaned_hold_reconciler.py`, `backend/tests/test_orphaned_hold_reconciler_coverage.py`, `docs/change-log/2026-09-23-orphaned-hold-lock.md`.
- [x] Test no reconciliation on unavailable/contended lock and recovery on next interval.
- [x] Import strict primitive, fail closed and emit counter. Preserve atomic authorization-state claims and cancellation keys.
- [x] Run orphaned-hold suites; commit `fix(payments): gate orphaned hold recovery on Redis`.
...
- [x] Test a due Sunday cycle cannot transfer on lock error, but can resume when acquisition succeeds. Final review also required gating the hourly stale-reserved transfer sweep; weekday and Sunday contention now skip both transfer paths.
- [x] Import strict primitive, fail closed and emit counter; retain payout claim/idempotency and payout settings.
- [x] Run auto-payout tests; commit `fix(payouts): require distributed ownership for automatic runs`.
...
- [x] Test contention/outage skips `run_autotopup_tick`, healthy next tick resumes; retain same-input Stripe idempotency tests.
- [x] Add strict SET NX EX before each tick, TTL 510 seconds below the 540-second minimum jittered interval; preserve heartbeat/error metrics.
- [x] Run corporate auto-topup tests; commit `fix(corporate): require Redis ownership for automatic topups`.
...
- [x] Test missing, unsupported and malformed runtime `REDIS_URL` blocks production, valid redis/rediss succeeds and development remains usable.
- [x] Validate the actual environment consumed by Redis utilities, separately from rate-limit storage; never echo credentials in errors.
- [x] Run production-config tests; commit `fix(config): reject production without utility Redis`.
...
- [x] Build/test a gate for exact SHA, repository, branch, CI completion/conclusion, required executed backend/security jobs, and current main. Missing evidence denies.
- [x] Integrate before secret staging/build/deploy; re-read main after polling. The production workflow runs on every main push with no manual bypass; signed-image deploy stays manual. Every push is necessary so a docs-only main advance cannot strand an older gated deploy.
- [x] Require readiness/served-SHA configuration for production and retain existing probes.
- [x] Run `python -m unittest discover -s scripts -p 'test_deploy_gate.py'`; commit each gate/integration slice independently.

### Task 10: API loop ownership and watchdog
Files: `backend/core/lifespan.py`, `backend/tests/test_lifespan_watchdog_coverage.py`, `docs/change-log/2026-09-23-loop-role-ownership.md`. First classify the existing route-deviation and insurance-period loops in `backend/utils/background_loop_registry.py` in a separate commit.
- [x] Verify all-role inventory stays intact, API role excludes only registry-designated worker loops, watchdog sees active loops only, dormant loops stay dormant.
- [x] Wire existing registry helpers into spawning/monitoring without activating a worker deployment.
- [x] Run lifespan/watchdog/worker suites; commit `fix(workers): align API loop ownership and watchdog`.
...
- [x] Test invalid weekly/monthly anchors and arbitrary underlying ValueError cannot leak its text, status remains 422 and no email is sent.
- [x] Replace exception interpolation with fixed actionable copy and redacted diagnostics.
- [x] Run statement-email tests; commit `fix(drivers): sanitize statement period validation errors`.
