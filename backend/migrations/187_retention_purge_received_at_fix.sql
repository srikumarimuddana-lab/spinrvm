-- 187_retention_purge_received_at_fix.sql
--
-- BUGFIX: purge_pii_retention() Step C referenced
-- driver_location_history.recorded_at, but that column does not exist on
-- production. Migration 08's CREATE TABLE defined `recorded_at`, but in
-- production the table pre-existed, so the `CREATE TABLE IF NOT EXISTS` was a
-- no-op and `recorded_at` was never created. The live ingestion path
-- (routes/websocket.py -> utils/breadcrumbs.py) writes `received_at` (added by
-- migration 118) and `timestamp` instead. The retention loop therefore failed
-- every tick with:
--     [DB] Supabase call failed (APIError): column "recorded_at" does not exist
--     ... Perhaps you meant to reference the column "...received_at".
-- meaning driver_location_history was NEVER purged at the 90-day floor — an
-- unbounded GPS-breadcrumb accumulation and a PIPEDA retention violation
-- (the 90d location-history floor was never enforced).
--
-- FIX: re-fork purge_pii_retention VERBATIM from migration 143 and change ONLY
-- Step C's two column references from `recorded_at` to `received_at`.
-- `received_at` (migration 118, ADD COLUMN IF NOT EXISTS DEFAULT NOW()) is
-- present in every environment and is the server-receipt timestamp written on
-- every breadcrumb — the correct equivalent of the original `recorded_at`.
-- Nothing else in the function changes.
--
-- Forward-compatible: CREATE OR REPLACE FUNCTION, no schema change, no lock on
-- driver_location_history beyond what the existing loop already takes at 03:00
-- UTC under its Redis leader lock. First successful run will delete the backlog
-- of >90d breadcrumbs in a single DELETE.
--
-- Rollback:
--   Re-apply migration 143's purge_pii_retention definition verbatim
--   (reverts Step C's two `received_at` references back to `recorded_at`).
...
BEGIN
    ...
    -- BUGFIX (migration 187): was `recorded_at` (nonexistent on prod); the live
    -- server-receipt timestamp column is `received_at` (migration 118).
    IF NOT p_dry_run THEN
...
COMMENT ON FUNCTION purge_pii_retention(BOOLEAN) IS
    'B-P1-6 + B-P1-7 + B-P2-4 + DV-8 retention enforcement. Uses actor_id (migration 57 schema). Step C uses driver_location_history.received_at (migration 187 fix; was the nonexistent recorded_at). Step I (migration 117, extended 129) clears ride_routes geometry at 3y. Step J (migration 141) deletes ai_messages/ai_conversations at 90d. Step K (migration 143) deletes surge_pricing history at 90d. p_dry_run=true returns counts without mutating.';
