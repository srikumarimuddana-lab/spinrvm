...
| `ENV` | Optional | `"development"` | Runtime tier — one of `development` \| `test` \| `staging` \| `production` (any other value refuses to boot, ADR-008). `staging` and `production` are "production-like": both enforce the strict startup guards (strong secrets, `ca-` Supabase region, CORS, cookie security). | Hard-code per deployment (`fly.toml` / `fly.staging.toml` / `fly.canary.toml`) |
| `DEPLOY_STAGE` | Optional | `"stable"` | `stable` or `canary`. The canary Fly app sets `canary` (with `ENV=production`) — used for the Sentry `deploy_stage` tag, logs, and `/health`; never for security decisions. | Set in `fly.canary.toml` only |
| `BACKGROUND_LOOPS_ENABLED` | Optional | `true` | When `false`, `core/lifespan.py` spawns none of the background loops (incl. the watchdog). Set `false` on the canary so money-mutating loops never run unpromoted code against prod data. | Set in `fly.canary.toml` only |
| `RELEASE_SHA` | Optional | `""` | Git SHA of the running build, injected by CI (`flyctl deploy --env RELEASE_SHA=…`). Surfaced at `/health` so the deploy pipeline can assert what is serving. | Set by CI |
| `EXPECTED_PROJECT_REF` | Optional | unset | `scripts/migrate.py` guard: refuses to migrate when this Supabase project ref doesn't appear in the configured DSN/URL. Set per GitHub Environment in CI. | Supabase project ref (the `xxxx` in `https://xxxx.supabase.co`) |
