/**
 * app/legacy-consent-notice.tsx (driver-app) — one-time re-consent notice.
 * Pins:
 *  - re-checks GET /consent/status on mount and redirects to /driver when
 *    needs_notice is false (self-heals if reached unnecessarily)
 *  - fails open to /driver when the status check itself throws
 *  - shows the notice and lets the driver accept via POST /consent/accept,
 *    then redirects to /driver
...
describe('LegacyConsentNoticeScreen (driver-app)', () => {
  it('redirects to /driver when needs_notice is false', async () => {
    mockApiGet.mockResolvedValue({ data: { needs_notice: false } });
    await renderScreen();
    expect(mockReplace).toHaveBeenCalledWith('/driver');
  });

  it('fails open to /driver when the status check throws', async () => {
  ...
  it('shows a toast and does not navigate when accept fails', async () => {
    ...
    expect(mockShowToast).toHaveBeenCalledWith('error', 'Something went wrong', 'Please try again.');
