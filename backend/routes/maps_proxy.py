...
async def _ensure_budget(sku) -> None:
    """Atomically reserve budget for one upcoming call to ``sku`` (C104).

    Replaces the old check_budget()-then-later-record_call(sku) pair: the
    reservation is folded into one atomic step here, right before the
    network call, so the caller must NOT also call record_call(sku) for the
    same attempt (see reserve_budget()'s docstring).
    """
...
async def places_autocomplete(
    ...
    # ending in Place Details Essentials — already recorded atomically by
    # _ensure_budget("autocomplete") above (C104).
