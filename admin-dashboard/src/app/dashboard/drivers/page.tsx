    ...
    return (
        <div className="bg-card rounded-xl border overflow-hidden transition hover:shadow-md group">
            <div className="relative h-44 bg-muted/50 flex items-center justify-center overflow-hidden">
                {isImage ? (<><img src={`/api/admin/documents/${d.id}/view`} alt={d.document_type||"Document"} className="w-full h-full object-cover" onError={e=>{(e.target as HTMLImageElement).style.display='none';}} /><button onClick={()=>onPreview(`/api/admin/documents/${d.id}/view`)} className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition flex items-center justify-center opacity-0 group-hover:opacity-100"><div className="bg-white/90 rounded-full p-2"><ZoomIn className="h-5 w-5 text-gray-800" /></div></button></>)
                : d.document_url ? <a href={`/api/admin/documents/${d.id}/view`} target="_blank" rel="noreferrer" className="flex flex-col items-center gap-2 text-muted-foreground hover:text-foreground transition"><FileText className="h-12 w-12 opacity-40" /><span className="text-xs font-medium">Click to view</span></a>
