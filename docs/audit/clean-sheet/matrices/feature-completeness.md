...
| Pickup confirmation (S-fulfil-03) | Y | Y | Y | Y | Y | Y | U | U | Y — a 4-digit pickup code gates trip start (`backend/routes/drivers/ride_flow.py:1164-1210`, lockout on failures); TSF-010 WITHDRAWN. Residual gap: rider-verifies-driver direction | U | U | N — CSV `has_test=False`; not independently re-checked | U | U | U | N | U | spinr-dispatch-reviewer (adjacent) | TSF-010 withdrawn 2026-09-25 after re-verification: code generated at `backend/dependencies/__init__.py:78-80`, shown in rider-app `driver-arriving.tsx`/`driver-arrived.tsx`, entered in driver-app `ActiveRidePanel.tsx`. Remaining: rider verifies driver; missing-code support path |
