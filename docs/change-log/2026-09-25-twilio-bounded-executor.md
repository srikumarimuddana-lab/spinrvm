...
| PR / commit link | Branch `claude/fix-twilio-bounded-executor`, not pushed and no PR yet. Commits: `2c176e1` (bounded pools), `31edd8a` (SOS pool), `125edfb` (general-pool sizing), `6f087fe` (contextvars), `f8c241c` and `7d12c7d` (SOS caller switch). |
...
| OTP `spinr-sms-otp` | `send_otp_sms`, from `/auth/send-otp` | 4 + 8 | One SMS per request, rate-limited to 6 per minute per client. At a sub-second Twilio round trip, 4 workers give 4+ sends per second (240+ per minute per replica). |
...
When a pool's workers and queue are all full, `BoundedExecutor.submit` raises `ExecutorSaturated`. `send_sms` catches it, logs an error bound with `sms_pool`, and returns `{"success": False, "provider": "twilio", "error": "ExecutorSaturated"}`.
...
- **Riders (OTP):** in normal operation, nothing changes. During a Twilio outage with more than 12 OTP sends in flight on one replica, an extra send returns the existing 503 "Failed to send verification code" immediately, instead of after up to 15 s.
...
|---|---|---|
| `backend/sms_service.py` | Adds 3 `BoundedExecutor` pools and `send_sos_sms`. `send_sms`, `send_otp_sms` and `send_sos_sms` all delegate to `_send_sms_on(executor, ...)`, which submits with `run_in_executor` inside a copy of the caller's context. Adds a fail-fast branch for `ExecutorSaturated`. | Keep Twilio outages off the shared default executor, and isolate SOS and OTP from each other and from broadcasts. |
...
- No `spinr-*` reviewer agent was run: that tool was not available in this session. It should be run before merge.
