...
| Breach end (confirmed, UTC) | `driver_bank_sin_migration.sql`: working-tree content blanked 2026-08-26T19:22:45Z (commit `44183d3`). `driver_csv_migration.sql`: working-tree removal, 2026-08-27 (PR #4731). **Update 2026-09-11:** git history itself has now also been rewritten (`git filter-repo`, both files stripped from every commit) and force-pushed to `main` and all 6 branches that had open PRs at the time — verified directly against GitHub post-push: zero commits touching either file remain reachable from any of those 7 refs. **Still not fully closed**, though, for the wider repo: an inventory taken the same day found the PII-touching commits still reachable from roughly 540 other branches (almost entirely stale, already-merged past Claude Code session branches, not open work) — those were deliberately left untouched in this pass to keep the force-push blast radius to what was actually live, and are a separate follow-up (see still-open item 1). |
...
| Fix deployed | Working-tree removal: `driver_bank_sin_migration.sql` blanked in commit `44183d3` (2026-08-26); `driver_csv_migration.sql` removed via PR #4731 (2026-08-27/30). **Git-history rewrite (2026-09-11):** owner explicitly authorized proceeding; `git filter-repo` run locally against a full mirror of the repo (`--invert-paths --path driver_bank_sin_migration.sql --path driver_csv_migration.sql`), verified zero remaining references across all history in the rewritten copy, then force-pushed to `main` and the 6 branches with open PRs at that time (`docs/ai-security-assessment-2026-09-08`, and 5 `dependabot/*` dependency-bump branches). Each of the 7 target refs was independently re-fetched and re-verified clean against GitHub after pushing, not just assumed from local script output. |
...
1. ~~Git history rewrite for both files (`git filter-repo` or BFG) — needs explicit owner authorization + coordination with all existing clones before executing.~~ **Partially closed 2026-09-11:** owner authorized the rewrite; executed via `git filter-repo` and force-pushed to `main` and all 6 branches that had open PRs at the time (see Fix deployed / Breach end above), each independently re-verified clean against GitHub. **Remaining gap:** an inventory taken the same day found the PII-touching commits still reachable from roughly 540 other branches on the remote — almost entirely stale, already-merged Claude Code session branches from past work, not active/open PRs. These were deliberately left out of the force-push to limit blast radius to what was actually live at the time. They still need either a rewrite-and-repush (same treatment as above) or outright deletion (their content is already merged into `main` in the vast majority of cases, so deletion loses little to nothing) before the exposure is fully closed. Separately, per GitHub's own guidance on purging sensitive data, GitHub Support should be asked to expire any cached views of the pre-rewrite commits/blobs — a force-push alone does not guarantee GitHub's internal caches are cleared immediately, and this has not been requested yet.
2. **Repository visibility (public → private) — re-verified 2026-09-11 via GitHub API: still `"private": false` / `"visibility": "public"`.** This is not a formality: the RROSH determination above rests explicitly on "the repository is being moved to private" as part of its rationale for not notifying OPC/affected drivers. It has now been **15 days** since that determination (2026-08-27 → 2026-09-11) and the repo remains fully public, meaning the exposure vector the determination assumed would be closed is still open today. This should go back to the Privacy Officer/owner promptly — either the visibility change happens, or the RROSH determination should be explicitly re-affirmed knowing the repo is still public, not left resting on a stated intent that hasn't materialized 15 days on.
