...
**This cannot be fixed by editing workflow YAML.** No branch-protection/ruleset API is exposed to any session (confirmed again on 2026-08-28 via `ToolSearch` for "repository ruleset settings admin" — nothing found). It requires a repo admin to:

1. Open **Settings → Branches → main** and compare the required-status-checks list against the full set of checks that actually run (`CI Guard Rails` summary, `Security Gates` summary, `backend-test`, `admin-test`, `rider-app-test`, `driver-app-test`, `Migration Safety Check`, `mobile-bundle-smoke`, and the path-filtered equivalents of all of those, now that Phase 2 below has landed). Add whatever's missing.
2. Decide whether **Settings → General → Allow auto-merge** should stay enabled repo-wide, given there is currently no *effective* human-review gate. **Update 2026-08-28 — real org data, not a placeholder guess:** checked via the GitHub API (`list_repository_collaborators`, `get_teams`) rather than assumed. This repo has exactly **2 collaborators** — `@srikumarimuddana-lab` (admin) and `@ittalenthireca-sketch` (write; per this repo's own commit/PR history, the account that opens/authors PRs, not an independent reviewer) — and **0 GitHub teams**. There is no one else to review a PR into. My recommendation is unchanged in substance: disable auto-merge repo-wide (except the already-correctly-scoped Dependabot patch-only path in `dependabot-auto-merge.yml`) until a second real reviewing account exists, or accept explicitly that `@srikumarimuddana-lab` is the sole gate for now and treat every merge as self-approved by design rather than by an unexamined default.
3. **Done 2026-08-28** — `CODEOWNERS` now routes every domain to the one real reviewing account above instead of the fictional `@spinr-org/TBD-*` placeholders (a `spinr-org` GitHub organization with those teams never existed). This makes the file *resolvable*; it does not by itself turn on enforcement — "Require review from Code Owners" is still a Settings → Branches → main checkbox item 1 above didn't separately call out but should: enable it once you're satisfied with the auto-merge decision in item 2, since a resolvable-but-unenforced CODEOWNERS file is the same practical risk as the placeholder version was.
