...
**Totals:** 142 rows, de-duplicated from 198 distinct finding-card ids (plus the decay-signal and ACTION_ITEMS ids they cite). Row severity: 6 CRITICAL, 41 HIGH, 6 MEDIUM-HIGH, 55 MEDIUM, 5 LOW-MEDIUM, 19 LOW, 4 RECOMMENDATION, 4 PASS/keep, 1 UNKNOWN, 1 WITHDRAWN. §10 lists the highest-priority rows; §11 is the id index.
...
| Row | Risk (plain language) | Merged IDs | Sev | S×B×L | Label | Owner role | Status | Roadmap |
|---|---|---|---|---|---|---|---|---|
| RR-42 | Float arithmetic on money is still recurring outside the lint gate: the canonical fare-estimate total (persisted for corporate guest rides), two driver tip totals, and referral bonuses. The pre-commit hook only warns; the CI rule has 7 exclusions and cannot see column types. | HIST-001, MONEY-006, MONEY-001, MONEY-008, MONEY-005 | HIGH | 4×4×3 = 48 | VERIFIED | BE-PAY | Open (class not closed; 5 new instances this wave) | N19, X5 |
...
| RR-50 | Corporate wind-down: the ledger debit has no idempotency key and the close guard has a check-then-act race, so a double close can debit a terminal account twice. | CORP-001 | HIGH | 4×2×2 = 16 † | VERIFIED | BE-PAY | Open | N20 |
...
| RR-53 | Money-moving admin actions (wallet credit/debit up to $10,000 each, dispute refunds up to the full fare) need no second approver and have no cumulative cap; the `support` and `finance` presets can reach them (a 10-per-minute rate limit exists). | ADMIN-OPS-001 | HIGH | 3×3×2 = 18 | VERIFIED | FOUNDER (threshold) → ADMIN | Decision needed | X7, E-F7 |
...
| RR-94 | The rider gets no event while a driver is deciding on the offer, although the dispatch doc promises one — a product decision, not a stranding bug. | DISPATCH-001 (W2 and rapid) | MEDIUM | 2×3×3 = 18 | VERIFIED | FOUNDER → BE-DISP | Decision needed | X13, E-F11 |
...
| RR-119 | No trip PIN or pickup identity check in either direction. | TSF-010, BENCH-001 | MEDIUM-HIGH (benchmark: HIGH) | 3×5×2 = 30 † | VERIFIED | FOUNDER → MOBILE + BE-DISP | Decision needed | X4 |
