...
| `refresh_tokens` | `revoked_at` + 30 days **or** `expires_at` + 30 days, whichever comes first | Hard DELETE | Tokens are already invalid at expiry; 30d grace lets `/auth/logout-all` forensics inspect recently-revoked sessions (B-P1-13). **Both arms matter:** `revoked_at` is only stamped by an explicit act (rotation, logout-all, reuse-cascade) — natural expiry stamps nothing, so the `expires_at` arm is what bounds an abandoned session. Between migrations 50 and 117 Step E narrowed to revoked-only and never-revoked tokens were retained indefinitely; migration 436 restored the second arm. |
