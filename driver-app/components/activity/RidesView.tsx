...
export default function RidesView() {
  ...
  const renderRideCard = ({ item }: { item: any }) => {
    ...
    const formattedDate = date
      ? new Date(date).toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })
      : '';
        ...
        <View style={styles.cardHeader}>
          ...
          <View style={{ alignItems: 'flex-end' }}>
            ...
            <Text style={styles.bookingIdText}>
              {item.ride_code ? item.ride_code : `ID: #${String(item.id).substring(0, 8).toUpperCase()}`}
            </Text>
          </View>
        </View>
          ...
          <View style={styles.fareContainer}>
            {isCompleted ? (
              <>
                ...
                <Text style={styles.fareText}>${(item.driver_earnings || item.total_fare || 0).toFixed(2)}</Text>
              </>
            ) : isCancelled ? (
              <Text style={[styles.fareText, { color: colors.textDim, fontSize: 16 }]}>$0.00</Text>
            ) : (
              <Text style={styles.fareText}>Est. ${(item.total_fare || 0).toFixed(2)}</Text>
            )}
          </View>
        </View>
      </TouchableOpacity>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.filterWrapper}>
        <FlatList
          ...
          contentContainerStyle={[styles.filterListContent, { marginBottom: 10 }]}
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyStateContainer}>
              ...
              <Text style={styles.emptyStateDesc}>There are no rides matching this filter criteria at the moment.</Text>
            </View>
          }
        />
      )}
