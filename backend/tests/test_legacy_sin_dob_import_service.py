...
def test_plan_matches_driver_directly_created_by_mongo_importer(monkeypatch):
    ...
    legacy_import_metadata.source == MONGO_IMPORT_SOURCE -- the SIN/DOB
    backfill must now match it, not just IMPORT_SOURCE (Saskatoon)
    drivers."""
    ...
    assert len(plan.updates) == 1
    assert plan.updates[0]["id"] == "spinr-driver-1"
