...
class PaymentIntentRequest(BaseModel):
    ...
    amount: Decimal = Field(
        ..., gt=0, le=100000, decimal_places=2, description="Amount in CAD (1–100000)"
    )
...
class PaymentSheetRequest(BaseModel):
    ...
    amount: Decimal = Field(
        ..., gt=0, le=100000, decimal_places=2, description="Fare amount in CAD"
    )
    ride_id: Optional[str] = None
    client_idempotency_key: Optional[str] = None
...
async def get_or_create_stripe_customer(user_id: str, stripe_secret: str):
    ...
    if not stripe_customer_id:
        ...
        await db_supabase.update_one(
            "users", {"id": user_id}, {"stripe_customer_id": stripe_customer_id}
        )
        ...
        if not fresh:
            raise HTTPException(
                status_code=404, detail="User not found during Stripe customer creation"
            )
...
async def create_payment_intent(
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail="Payment service unavailable. Please try again."
        ) from e
    ...
    if stripe_secret:
        try:
            ...
            if intent.metadata.get("user_id") != str(current_user["id"]):
                raise HTTPException(
                    status_code=403, detail="Not authorized to confirm this payment"
                )
...
async def create_setup_intent(
    ...
    try:
        ...
        if not customer_id:
            raise HTTPException(
                status_code=400, detail="Could not create Stripe customer"
            )
...
async def get_cards(
    ...
    try:
        ...
        methods = stripe.PaymentMethod.list(
            customer=customer_id, type="card", api_key=stripe_secret
        )
        user = await db_supabase.get_user_by_id(current_user["id"])
...
async def add_card(
    ...
    try:
        ...
        pm = stripe.PaymentMethod.attach(
            payment_method_id, customer=customer_id, api_key=stripe_secret
        )
        ...
        logger.info(
            f"Card added: {pm.card.brand} ****{pm.card.last4} | SetupIntent: {si.id} ({si.status})"
        )
...
async def set_default_card(
    ...
    await db_supabase.update_one(
        "users", {"id": current_user["id"]}, {"default_payment_method": card_id}
    )
    ...
    if stripe_secret:
        try:
            ...
            logger.error(
                f"Stripe set default failed for card {card_id}: {e}", exc_info=True
            )
...
async def delete_card(
    ...
    if user and user.get("default_payment_method") == card_id:
        await db_supabase.update_one(
            "users", {"id": current_user["id"]}, {"default_payment_method": None}
        )
...
async def create_payment_sheet(
    ...
    if body.ride_id:
        ...
        if ride.get("rider_id") != current_user["id"]:
            raise HTTPException(
                status_code=403, detail="Not authorized to pay for this ride"
            )
        ride_fare = Decimal(str(ride.get("total_fare", 0)))
        requested = Decimal(str(body.amount))
        if requested != ride_fare:
