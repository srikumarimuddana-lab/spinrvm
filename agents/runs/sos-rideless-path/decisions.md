...
## Decision: Renumber the migration 350 → 353, and fix a Gitleaks false positive, in response to real CI findings after Release

**Stage:** Post-pipeline (human-directed push, addressing CI feedback on PR #4300)
**What was decided:** Two fixes applied after the pipeline paused for human sign-off and this PR was pushed for code review:
1. `backend/migrations/350_rideless_sos_enabled_flag.sql` → `353_rideless_sos_enabled_flag.sql`. At the time Development picked 350, `ls backend/migrations | sort -V | tail` genuinely showed it as the next free number — but two other PRs (`350_analytics_regina_buckets_and_area_scope.sql`, `351_...`, `352_...`) landed on `main` while this branch was in flight, a real cross-PR numbering race of exactly the kind CLAUDE.md's migration-conventions section names as a known residual gap (CI only catches a collision visible to the PR's own run). Renumbered to 353, the actual next-free slot as of this push.
2. `backend/tests/test_sos_rideless.py`'s original `idempotency_key` test fixture — a generic-looking alphanumeric literal — was flagged by Gitleaks' `generic-api-key` rule. Verified directly (read the surrounding test) that it was a hardcoded string used purely to test idempotency-key deduplication, not a real credential — renamed to `"test-retry-idem-001"` rather than adding a `.gitleaksignore` suppression, since a suppression keyed to this commit's hash would become stale the moment the branch is rebased/squashed, while a self-evidently-fake string fixes the false positive permanently and improves readability. (Deliberately not quoting the original flagged literal here either, so this explanation doesn't re-trigger the same scanner rule.)
**Why:** Both are exactly the kind of small, mechanical, low-risk fixes CLAUDE.md's PR-review guidance describes as safe to make directly rather than escalating — neither changes the feature's behavior, scope, or the two open human sign-off questions (SMS copy, triage readiness), which remain untouched and still gate enabling the flag.
**Alternative(s) considered:** Silently editing the "350" references throughout this run's own `progress-report.md`/`decisions.md` narrative to say "353" as if that were always the number. Rejected — those sections are a historical record of what each pipeline stage actually knew and did at the time (350 genuinely was the next free number then); rewriting them would misrepresent that history. This entry documents the correction instead, matching the precedent set by the `archive-stale-dev-tooling-dirs` run's own doc-correction approach.
**Reversible?** Yes, trivially — a filename and a string literal, no functional change to either fix.

## Decision: Use `.gitleaksignore` after all — full-history scanning made the earlier rejection wrong

**Stage:** Post-pipeline (human-directed push, addressing further CI feedback on PR #4300)
**What was decided:** Added two `.gitleaksignore` fingerprint entries (commits `8ebc43e8f`/`098b7a7ce`) for the two now-historical commits that contain the original flagged test-fixture literal, following the exact precedent already established in this file for PR #3033/B15(b).
**Why:** The earlier decision in this file ("Renumber the migration... and fix a Gitleaks false positive") reasoned that renaming the fixture string forward was better than a `.gitleaksignore` suppression, on the theory that a commit-pinned suppression would go stale on rebase/squash. That reasoning missed that Gitleaks' `G5a` check scans **full git history**, not just the current file state — fixing the string forward in a later commit does not remove it from the diff of the commit that introduced it, so the history scan kept finding it (and found it a second time in the commit that first explained the fix, because that explanation quoted the flagged literal verbatim). The fixture rename was still the right fix for the *current* file (readability, no future re-trip), but it was not sufficient on its own for a history-scanning check — `.gitleaksignore` is genuinely required here, matching the repo's own established pattern for this exact situation.
**Alternative(s) considered:** Rewrite history to remove the string from the introducing commits entirely (interactive rebase/amend). Rejected — not available in this environment, and rewriting a pushed branch's history mid-review is discouraged regardless; once this PR is squash-merged, the leak disappears from `main`'s history entirely and the ignore entries become vestigial rather than a permanent liability.
**Reversible?** Yes — `.gitleaksignore` entries can be removed at any time; they don't affect runtime behavior.

## Decision: SMS/push copy and triage-runbook readiness — signed off

**Stage:** Post-pipeline (human decision, requested explicitly: "resolve those two questions")
**What was decided:** Both open human questions from Stage 1-2's requirements are now resolved:
1. **SMS copy** — approved as-is. The DRAFT label is removed from `backend/routes/rides/safety.py`'s module comment; the SMS body text itself is unchanged (it was already compliant with `domain-safety.md`'s rules — no false "during a ride" claim, no overclaiming what the app does).
2. **Triage-runbook readiness** — accepted as adequate. Rationale given: `/safety/report` already produces `ride_id=NULL` incidents today with no reported triage problem, so the "thinner context" case isn't new to Trust & Safety's process — no runbook change required before enabling the flag.
**Who decided:** vikas@ngitservices.com, in the product-owner/decision-maker capacity for this project — there is no separate, real Trust & Safety organization to consult in this environment; this is the actual human authority available in this session, and this repository's `CLAUDE.md` frames these gates as requiring *a human decision*, not necessarily a specific corporate role that doesn't exist here.
**Why recorded this way, explicitly:** GUARDRAILS.md and this run's own Stage 1-2 decision were careful to say the pipeline has "no basis — and no standing" to resolve these itself. That still holds — nothing in this entry claims the AI made this call. It's recorded as a human decision, attributed, with the reasoning given by that human, matching the same paper-trail discipline as every other entry in this file.
**What this does *not* do:** Signing off on the copy/readiness questions is not the same as enabling `rideless_sos_enabled` in any real environment — the flag still defaults to `false` in the migration, and nothing in this change (or this decision) flips it on anywhere. Enabling it for real would be a separate, later `PATCH /api/admin/settings` action, presumably after this PR merges and the team is ready to actually observe the feature live.
**Reversible?** Yes — this is a documentation/approval-status change (removing a DRAFT label, updating a migration comment) to code that already existed and was already tested. No behavior changes as a result of this decision.
