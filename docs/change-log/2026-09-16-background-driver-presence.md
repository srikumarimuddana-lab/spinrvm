...
Files: driver-app/utils/backgroundLocation.ts, its utils/__tests__/backgroundLocation.test.ts, this log. Consumers: the registered Expo background location handler; no foreground watcher or fare-capture changes. Before: every active-iOS callback could start both network paths. After: each path reserves at most one foreground slot per four seconds and prevents overlapping in-flight work. Alternative of dropping native samples was rejected because it would lose trip history. Risk: foreground delivery may wait for the next callback; the durable recorder retains skipped-upload samples. Rollback: disable stationary tracking via admin settings and resume after cache expiry; reverting pacing itself requires a mobile release.

Verification: 140 tests passed across six mobile suites including concurrent callbacks, slow history, slow App Check, background bypass, auth/session lifecycle and rollout options. Dispatch timestamps are refreshed immediately before fetch so slow authentication cannot bunch requests. TypeScript passed; mobile ESLint zero errors with 10 test warnings. Admin production Webpack build and all 9 settings browser tests passed (Chromium 1234 substitute for unavailable1243; mocked backend and realistic test token expiry). Real-device battery/OS timing, live Supabase migration and live flag rollout remain unverified.
