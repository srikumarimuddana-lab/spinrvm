...
## Fix
The workflow previously treated missing `STAGING_URL` as success, did not validate the scan report, and allowed ZAP findings to leave the action green. Require both `STAGING_URL` and `STAGING_ALLOWED_ORIGIN`; validate their exact HTTPS-origin match; generate an anchored ZAP context include regex and pass it to baseline with `-n` before spidering. Configure findings as failures, retain raw reports on failed scans, and require nonempty valid JSON whose every site is that same host, port, and TLS state.
...
## Before / after
Before, missing target exited successfully, any scan target could spider beyond the intended origin, and findings could be green. After, missing/mismatched configuration fails before ZAP; a context restricts the spider to the allowlisted origin, ZAP WARN/FAIL findings fail the job, raw reports remain available, and absent/malformed/mixed-target report fails the job after scanning.

## Rollback plan
Revert the isolated helper commit; it does not affect runtime or persisted state.

## Verification performed / not verified
- [x] `/tmp/pr5725-venv/bin/python -m pytest scripts/test_validate_dast.py -q` — 8 passed after severity follow-up; `git diff --check` clean.
