...
We encrypt sensitive information such as driver license numbers and emergency-contact details at rest. Access to personal information inside Spinr is limited to staff who need it to do their job — for example, our support team can see the information needed to resolve your ticket, not your full account history by default. From time to time, a small number of senior administrators may also move a limited set of account and trip records — including driver identity documents and precise trip location history — between our own internal systems, for example to test or maintain the platform. This never involves sharing your information with an outside party: it stays entirely within our own systems, and each transfer must have a recorded business reason. If we ever become aware of a data breach that poses a real risk of harm to you, we will notify the Office of the Privacy Commissioner of Canada within the timeframe PIPEDA requires and notify you directly as soon as reasonably possible, explaining what happened, what information was involved, and what we're doing about it.
...
4. **Scheduled deletion enforcement (DV-8) — closed 2026-08-10 (corrected 2026-08-27).** This policy promises deletion "within 30 days" and the specific longer regulatory-retention periods. The enforcing job now exists and runs: `retention_purge_loop` is spawned every ~24h on every backend replica (`backend/core/lifespan.py`) and calls the `purge_pii_retention()` Postgres function, whose horizons are built across migrations 50–296 — including the 30-day profile scrub anchored on `deletion_requested_at` (`backend/migrations/296_pipeda_30day_profile_scrub.sql`, Step N) that this promise depends on (tracked closed as `ACTION_ITEMS.md` B18). The earlier "no job exists — do not publish" note was stale; the retention promises are now backed by code. (Independent of this: the DPA gap in note #1 still gates full publication.)
...
6. **Internal cross-environment data movement disclosure (new, 2026-09-11)** — section 6's new sentence ("a small number of senior administrators may also move a limited set of account and trip records... between our own internal systems") discloses the Data Transfer module (`backend/routes/admin/data_transfer_export.py`), added here per the PIA determination at `docs/privacy/2026-07-28-pia-data-transfer-export.md` Section 8, R-G (resolved 2026-09-08 — option (a), a distinct disclosure is required) and `reports/legal/data-transfer-implied-consent-review.md`. Every factual claim in the new sentence is verified against the module's current code, not assumed: access is `super_admin`-only (`docs/change-log/2026-07-28-data-transfer-router-super-admin-gate.md`), a `reason` field is required on every export request (`docs/change-log/2026-07-28-data-transfer-export-reason-field.md`), and the data never leaves Spinr's own Supabase project (PIA Section 6 — no third-party recipient). This is a **draft addition only** — per `reports/legal/data-transfer-implied-consent-review.md`'s Status table, the live, already-published policy text should get this sentence at its next full reviewed/re-published version, not as a standalone unreviewed edit to the text already in production. Tracked in `docs/legal/legal-text-publication-checklist.md`'s `privacy-policy.md` row.
