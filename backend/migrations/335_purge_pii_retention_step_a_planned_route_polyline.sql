-- 335_purge_pii_retention_step_a_planned_route_polyline.sql
--
-- BUGFIX: purge_pii_retention()'s Step A anonymizes ride GPS at 3 years by
-- nulling pickup_lat/lng, dropoff_lat/lng, and clearing route_polyline,
-- phase_polylines, and route_snapshot_url -- but never touches
-- rides.planned_route_polyline (migration 100), the Google Directions
-- polyline captured at booking time. A ride older than the 3-year GPS
-- anonymization window still has its full planned turn-by-turn route
-- ([[lat,lng], ...]) sitting live in that column even after
-- gps_anonymized_at is stamped, contradicting both the column's own GPS
-- nature and regulatory-sk.md's "GPS trace at pickup and dropoff... 3
-- years" retention rule this step exists to enforce.
--
-- Found 2026-08-19 via A40 (docs/audit/2026-08-18-full-fleet-whole-app-audit.md,
-- ranked blocker #4/#11) -- confirmed by reading the current live function
-- body (migration 324, still the latest CREATE OR REPLACE as of this
-- migration) and grepping every migration after 100 for
-- planned_route_polyline: only 313 (an unrelated legacy-shape repair, not a
-- purge step) touches it.
--
-- FIX: re-fork purge_pii_retention VERBATIM from migration 324 (current
-- version, includes 321's Step H driver-ride guard and 323/324's Step
-- D/F column fixes) and add ONLY
-- `planned_route_polyline = '[]'::jsonb` to Step A's anonymizing UPDATE --
-- same empty-array sentinel migration 100 uses as the column's own DEFAULT,
-- and the same shape route_polyline/phase_polylines already reset to on
-- this same step. Nothing else changes.
--
-- Forward-compatible: CREATE OR REPLACE FUNCTION only, no schema change
-- (the column and its type already exist, migration 100), no data
-- mutation beyond what Step A's existing WHERE clause already targets, no
-- new lock beyond what the existing 03:00 UTC loop already takes under its
-- Redis leader lock. Idempotent: Step A's own
-- `WHERE ... AND gps_anonymized_at IS NULL` guard is unchanged, so a ride
-- already anonymized under the old function body is NOT retroactively
-- re-touched by this fix -- see "What was NOT verified" in this session's
-- change-log for that explicitly-accepted gap.
--
-- Rollback:
--   Re-apply migration 324's purge_pii_retention definition verbatim
...
BEGIN
    ...
    -- (migration 100) added here -- see this migration's own header.
