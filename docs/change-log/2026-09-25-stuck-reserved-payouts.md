...
- `payout_type='auto'`: not covered by this check (see the scope choice below). Standard and instant Transfers carry **no** metadata, so for those rows the idempotency key (in Stripe's request logs) or destination account + amount + time are the only ways to find the Transfer.

**Scope choice (revised after `spinr-money-auditor` review).** The check covers only `standard` and `instant` rows. The query filters `payout_type IN ('standard','instant')`, and the loop also skips `auto` defensively. Auto rows are left out because `auto_payout.sweep_stale_reserved` already retries or escalates stale reserved auto rows every hour, and an escalated row keeps `status='reserved'` (only `failure_reason` changes). Including them would have re-logged every escalated auto row at error level every day, forever, and could have crowded new standard or instant rows out of the 50-row `discrepancy_detail` sample. A `payout_type` outside the two covered values is labelled `other`, and `other` is pre-registered at 0 too. `payout_type` defaults to `'standard'`, and migration 138 back-filled NULLs, so a NULL-type row is not expected.
...
- Metrics: the existing `_CHECK_NAMES` gains `stuck_reserved_payout`, which pre-registers one more zero series. One new counter family is added, pre-registered at 0 for `standard`, `instant` and `other`, following the module's convention so that `increase()` sees the first real hit. The name uses the singular `spinr_payment_` prefix, like its sibling metrics and CLAUDE.md's `spinr_payment_settlement_total`.
...
- If the log volume is noisy: filter or silence `PAYOUT_STUCK_RESERVED` in Sentry. No deploy is needed.
...
**Addendum (review follow-up).** `spinr-money-auditor` gave the verdict SAFE TO MERGE with one should-fix, auto-row noise, and one nit, the unregistered `other` label. Both are fixed in the follow-up commit. It adds 2 tests: auto rows left to the sweep, including the `in_('payout_type', ['standard','instant'])` filter assertion; and an unexpected type labelled `other`. The new file now has 14 passing tests; the full `stripe_reconcile` set plus the loguru-convention tests give 142 passed.