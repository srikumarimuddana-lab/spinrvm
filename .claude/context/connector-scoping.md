...
|---|---|---|---|---|
| Supabase | Account-level | MCP URL params `?project_ref=<id>&read_only=true`, or a project-scoped read-only PAT | **Needs narrowing** — connector exposes `execute_sql`/`apply_migration`/`pause_project` against `Spinr-Prod` (`cfrazforbupizntxvvtp`); org also contains `MobileAppStaging` (`mvmyygoinicjdpqprizr`), no third-party project found | 2026-09-07 |
