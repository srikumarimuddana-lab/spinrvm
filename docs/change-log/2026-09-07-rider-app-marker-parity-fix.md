...
- [x] `npx expo export --platform web` run after the change — completed successfully (exit 0, `dist/` bundle produced, no errors). This confirms the diff bundles cleanly but is **explicitly not a substitute for a real device/native build** per this repo's own PR template ("`expo export --platform web` + the full JS test suite are not a substitute for a real native build") — no new native dependency, Expo SDK version, or native module was added or changed by this PR (it only changes JS-side logic calling the same `react-native-maps` APIs already used elsewhere in this file), so the template's native-build-verification requirement does not gate this specific PR, but this is a JS-bundle smoke test, not a rendering/behavior confirmation.
