...
**33 entries**, audience `driver`. Seeded by `backend/migrations/212_seed_saskatchewan_driver_faqs.sql`. Three of those 33 were later deactivated: the SOS/safety-features entry, consolidated into a shared `audience='both'` FAQ (see the note in the Safety & coverage section below), and two near-duplicate onboarding-status entries, merged into their migration-210 equivalents by `backend/migrations/327_merge_duplicate_onboarding_faqs.sql` (see the notes in this section below) — leaving 30 active driver-only entries from this migration.
