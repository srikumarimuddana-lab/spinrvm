...
// Carto basemap — keyless, hosted on a different CDN from OpenFreeMap.
// Last-resort fallback when both OpenFreeMap and Protomaps are unreachable.
export const MAP_STYLE_CARTO_LIGHT = "https://basemaps.cartocdn.com/gl/positron-gl-style/style.json";

/** Self-hosted tile server style URL, or null if not configured. */
export function selfHostedStyleUrl(): string | null {
    const url = process.env.NEXT_PUBLIC_MAP_STYLE_URL?.trim();
    return url || null;
}

/**
 * Build the ordered basemap provider chain. Tries self-hosted first,
 * then OpenFreeMap (themed), then Protomaps (if key set), then Carto.
 * Deduplicated so a provider appearing in two slots doesn't waste a retry.
 */
export function basemapChain(resolvedTheme?: string): string[] {
    const sh = selfHostedStyleUrl();
    const ofm = themedMapStyle(resolvedTheme);
    const pm = protomapsStyleUrl();
    return [...new Set([
        ...(sh ? [sh] : []),
        ofm,
        ...(pm ? [pm] : []),
...
interface BasemapFallbackHandlers {
    onLoaded: () => void;
    onRetry: (nextUrl: string, nextAttempt: number) => void;
    onExhausted: (reason: string) => void;
}

/**
 * Watch a MapLibre map for load success / failure and walk the basemap
 * chain on timeout. Returns a detach (cleanup) function.
 */
export function attachBasemapFallback(
    ...
    const tryNext = () => {
        ...
        if (next < chain.length) {
            handlers.onRetry(chain[next], next);
        } else {
            handlers.onExhausted("All basemap providers failed to load");
        }
    };

    map.once("load", succeed);
    const timer = setTimeout(tryNext, timeoutMs);

    return () => {
        settled = true;
        clearTimeout(timer);
        map.off("load", succeed);
    };
}
