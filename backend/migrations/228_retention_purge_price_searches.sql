-- 228_retention_purge_price_searches.sql
-- Rollback:
--   Re-apply migration 216's purge_pii_retention definition verbatim
--   (drop Step L, the v_ps_anonymized / v_ps_deleted counters, the
--   c_ps_anon_age / c_ps_keep_age constants, and the two
--   'price_searches_*' jsonb keys).
--
-- price_searches (migration 226) is an append-only funnel-analytics log
-- written fire-and-forget on every rider fare estimate. It stores user_id
-- (attributable) + service_area_id and was not covered by any retention
-- step — unbounded growth and a PIPEDA data-minimization gap (user_id
-- retained forever, surviving account deletion).
--
-- Step L (added here):
--   L1 — ANONYMIZE user_id at 90 days (c_ps_anon_age, matching the Step C/D
--        90-day PII horizon). The funnel only ever COUNT(*)s the table, so
--        nulling user_id loses nothing; an anonymized row (timestamp + area
--        only) is no longer personal information.
--   L2 — DELETE rows at 25 months (c_ps_keep_age). The /earnings/overview
--        funnel's YTD period compares against a same-length PRIOR window, so
--        late-December YTD reaches back ~24 months; 25 months keeps the
--        comparison exact, then rows drop entirely to bound table size.
--
-- No Step H change is needed for DSAR: Step H fires >= 7 years after the
-- deletion request, and Step L has anonymized every price_searches row long
-- before that (a deactivated account creates no new searches), so no
-- attributable row can survive to the hard-delete.
--
-- Re-forks purge_pii_retention VERBATIM from migration 216 (the current
-- authoritative definition — includes the 216 Step H hard-delete model) and
-- adds ONLY Step L + its counters. The existing retention_purge_loop picks
-- this up with no Python change.
