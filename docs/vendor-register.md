...
| **Sentry** (error telemetry) | Error tracking & performance monitoring | Error stack traces (PII-scrubbed in `beforeSend`), anonymised session replays | [Sentry DPA](https://sentry.io/legal/dpa/) | ⚠ US ingestion (default SaaS) — **open item: migrate to Sentry EU or file DPA addendum** (A-PE-P2-5) | _TBD_ | Annual | `blockAllMedia: true`. All PII keys filtered. URL path IDs scrubbed. Canadian admin data going to US is a PIPEDA residency question pending resolution. |
...
| Resolve Sentry US ingestion — migrate to EU or DPA addendum (A-PE-P2-5) | Engineering + Legal | Q2 2026 |
