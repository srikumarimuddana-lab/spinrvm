...
test.describe('admin dashboard: support — interaction', () => {
  ...
  for (const tab of ['Disputes', 'Complaints', 'Lost & Found', 'Flags', 'FAQs', 'Legal']) {
    test(`${tab} tab switches without crashing`, async ({ page }) => {
      ...
      await page.goto('/dashboard/support');
      await expect(page.getByRole('heading', { name: 'Support & Issues' })).toBeVisible({ timeout: 20000 });
      // IA audit Finding G: the tab strip is now a real Radix Tabs
      // component (role="tablist"/"tab", URL-synced via ?tab=), not a
      // custom button row — and Finding A removed tickets.tsx's own
      // nested "Tickets | FAQs" sub-tab bar (a 3rd, undocumented FAQ
      // implementation), so there's no longer a second "FAQs" control on
      // this page to disambiguate against.
