...
export default function HomeScreen() {
  ...
  const [nearbyDrivers, setNearbyDrivers] = useState<{ id: string; lat: number; lng: number; heading?: number; vehicleTypeName?: string }[]>([]);
      ...
      api.get(`/drivers/nearby?lat=${latitude}&lng=${longitude}`)
        .then((res: any) => {
          const drivers = (res.data || []).map((d: any) => ({
            ...
            vehicleTypeName: d.vehicle_type_name ?? undefined,
          }));
  ...
  const displayDrivers = useMemo(() => {
    ...
    const out: { id: string; lat: number; lng: number; heading: number; vehicleTypeName?: string }[] = [];
    for (const group of groups.values()) {
      group.forEach((d, i) => {
        ...
        out.push({ id: d.id, lat, lng, heading, vehicleTypeName: d.vehicleTypeName });
      });
    }
    return out;
  }, [nearbyDrivers]);
          ...
          >
            {displayDrivers.map((driver) => (
              <CarMarker
                ...
                vehicleType={driver.vehicleTypeName}
                size={32}
              />
            ))}
          </AppMap>
