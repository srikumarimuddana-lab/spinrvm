...
# ── Dispatch latency by zone ───────────────────────────────────────────


@api_router.get("/dispatch-latency")
async def get_dispatch_latency(
    date_range: str = Query("30d", pattern="^(today|7d|30d|90d|1y)$"),
    service_area_id: Optional[str] = None,
    admin: dict = Depends(get_admin_user),
):
    ...
    CLAUDE.md's Performance SLA table names this KPI (target: P95 < 2s), and
    it IS measured today via the spinr_dispatch_offer_to_accept_duration_ms
    Prometheus histogram (routes/drivers/ride_flow.py) -- but that only
    reaches Grafana, never this dashboard, and is never broken out by zone.
    This reconstructs the same offer->accept delta from durable storage
    (ride_offers.offered_at/responded_at) instead, so a specific zone's
    ...
    cache_key = f"analytics:dispatch-latency:v1:{date_range}:{service_area_id or 'all'}"
    cached = await redis_get(cache_key)
    if cached:
        try:
            return _json.loads(cached)
    ...
    try:
        dl = await db.rpc(
            "admin_dispatch_latency_by_zone",
            {
                "p_start": start_date.isoformat(),
                "p_end": now.isoformat(),
                "p_service_area_id": service_area_id,
        ...
        logger.error(
            f"Failed to aggregate dispatch latency: {e}",
            exc_info=True,
            extra={"domain": "admin"},
        )
        raise HTTPException(status_code=503, detail="analytics_unavailable") from e

    dl = dl[0] if isinstance(dl, list) and dl else dl
    ...
    result = {
        ...
        "p50_ms": float(dl.get("p50_ms") or 0),
        "p95_ms": p95_ms,
        # A window with zero accepted offers gets percentile_cont(NULL) from
        # Postgres, not 0 -- if the KPI were reported here anyway, `or 0`
        # above would read as "0ms, comfortably meeting the 2s target" for a
        # zone that has no dispatch signal at all, a false positive rather
        # than an honest "no data". Match the by_zone convention below:
        # omit rather than zero-fill.
        "kpis": [_kpi("dispatch_p95_ms", p95_ms)] if sample_count > 0 else [],
        # Per-zone breakdown — the point of this endpoint. Empty samples per
        # zone are omitted by the SQL function's GROUP BY, not zero-filled,
