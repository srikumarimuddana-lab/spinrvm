...
6. **If there's no *active* automated visual/snapshot regression tooling for the surface you're touching, say so explicitly** rather than silently relying on "no visible diff" reasoning. rider-app and driver-app have none at all. admin-dashboard has a real, CI-wired Playwright visual-regression job (`e2e/visual-regression.spec.ts`) and, **since PR #4916 (`27ff638`, 2026-09-02), it has committed baselines and actually runs** — 5 of 6 are seeded (`login`, `dashboard-home`, `dashboard-drivers`, `dashboard-monitoring`, `dashboard-settings`). This paragraph previously said the opposite ("zero committed baseline screenshots, so the job … skips itself on every run"); that was true when written and is not any more. **Do not tell a reviewer an admin-dashboard change has no visual coverage — check which page you touched.**
    - `dashboard-rides` is still unseeded, so that one test fails on a missing snapshot rather than a diff. `visual-regression-test` in `ci.yml` therefore keeps `continue-on-error: true` until all 6 exist, which is why the job can report a red check while the workflow run still concludes `success` — a red visual-regression check is **not** currently merge-blocking, and is also not nothing. See `ACTION_ITEMS.md` B38 for the live status; prefer it over this paragraph if the two ever disagree again.
