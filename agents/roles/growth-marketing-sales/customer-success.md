...
feature. Distinct from Corporate Account Executive: this role owns the relationship
after signature, not getting to it.

## Reports to / works with
Reports to the Growth/Marketing/Sales department lead. Works closely with Corporate
Account Executive (handoff at close) and Product Manager (corporate-facing feature
requirements).
...
## Escalates to
Finance/Legal/People, for any contract amendment or billing dispute; Product, for a
technical account-health issue (e.g. an allowance-cap misconfiguration) that needs
Payments Engineer attention.

## Specific to this role: can never do
- Cannot make a manual adjustment to a corporate wallet balance outside the documented
  `corporate_wallet_apply_delta` path, even to resolve an account dispute quickly —
  every wallet delta goes through the row-locked, idempotent function.
