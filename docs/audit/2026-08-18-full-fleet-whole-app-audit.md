...
| N9 | ~~Fare-estimate endpoint can hard-block up to 3.5s waiting on Google Directions before fallback — a real, code-confirmed >10x breach of the 300ms SLA target, by design, undocumented in the SLA table~~ | performance | `routes/rides/estimates.py:62,455` | **RESOLVED 2026-08-21** — behavior confirmed still live (widened 1.5s→3.0s on 2026-07-29 after a pricing-inconsistency incident); documented in CLAUDE.md's SLA table (2026-08-19). The accept-vs-ceiling decision is now closed: Option A (accept as permanent exception), Product/Eng lead sign-off 2026-08-21. |
...
| 24 | ~~Fare-estimate 3.5s Directions wait undocumented in SLA table~~ | performance | Eng — Pricing + doc owner | **RESOLVED 2026-08-21** — documented in CLAUDE.md's SLA table (2026-08-19); ceiling-vs-accepted-exception decision closed 2026-08-21, Option A (accept) |
...
| 30 | ~~Money-path coverage floors unenforced beyond corporate_*~~ | test-coverage | Eng — Backend/QA | **RESOLVED 2026-08-19** — new blocking gate for `payments.py`/`fare_service.py`/`crypto.py`/`rides/`/`dispatch_service.py`, floors set just below each file's real measured coverage. **Escalation now closed 2026-08-19**: `payments.py` measured 86.1% at the gate's introduction, below its documented 90% target — 14 targeted unit tests added (`test_payments_coverage_gap_closure.py`) raised it to 90.16%, crossing the target; the gate's floor updated to 85% (measured-5) matching the other "target met" rows. See `docs/change-log/2026-08-19-money-path-coverage-floor-gate-fix.md` and `docs/audit/2026-08-19-decision-writeups.md` item #4. |
...
| ~~Emergency-contact encryption: implement pgcrypto vs. accept plaintext (doc already corrected to state actual behavior)~~ — **DECIDED 2026-08-21**: encrypt, mirroring migration 32's pgsodium/Vault pattern (not pgcrypto — no such extension exists in this repo). See `docs/audit/2026-08-21-emergency-contact-pia-memo.md` (Recommendation 1) and migration 357. | safety-sos, security | Privacy/Legal | Closed 2026-08-21 |
...
~~Per-module coverage gate scope: extend `check_corporate_coverage_floor.py`'s pattern to `payments.py`/`fare_service.py`/`crypto.py`/`rides.py`/`dispatch_service.py`~~ — **ACTIONED 2026-08-19**, gate now exists. ~~Residual decision: `payments.py` measured 86.1%, below its documented 90% target~~ — **RESOLVED 2026-08-19**, tests written (see ranked blocker #30 above), now 90.16%, target met | test-coverage | Eng lead / QA owner | Closed 2026-08-19 |
| ~~Migration numbering collisions: 7 duplicate prefixes now exist (up from 3) — accept as documented convention or invest in a merge-queue/stronger CI check~~ — **DECIDED 2026-08-21**: enable GitHub's native merge queue (Option B). CI-side prep shipped (`merge_group:` trigger added to CI/CD Pipeline, CI Guard Rails, Security Gates, Migration Safety Check — commit `0e8fbee`); the repo-settings toggle itself (Settings → Branches → "Require merge queue") needs a repo admin — no GitHub MCP tool in this environment exposes rulesets/branch-protection administration. | migration | Eng lead | Decided 2026-08-21 — toggle pending repo-admin action |
...
| ~~Fare-estimate 3.5s Directions-wait trade-off: document as an accepted SLA exception, or add a hard latency-preserving ceiling and accept the undercharge-risk tail~~ — **DECIDED 2026-08-21**: accept as a permanent, documented SLA exception (Option A). No code change — `_PRICING_ROUTE_WAIT_S` unchanged. See `docs/audit/2026-08-19-decision-writeups.md` item #8. | performance | Product/Eng lead | Closed 2026-08-21 |
