...
| `backend/migrations/418_settings_driver_turn_by_turn_enabled.sql` | New: adds the matching `settings` column | Required pairing per `test_settings_column_parity.py` — without it, setting the flag 500s the whole save |
...
**Second renumbering (same root cause, this PR stayed open long enough to hit it twice):** while this PR was still open, `main` merged a further unrelated migration, `417_webhook_preauth_failure_ack_flag.sql` (a Stripe pre-auth webhook fix, no overlap with this PR). On the next `git merge origin/main` into this branch, this PR's own `417_settings_driver_turn_by_turn_enabled.sql` was renamed again to `418_settings_driver_turn_by_turn_enabled.sql`, with the same set of cross-references updated (migration header, `settings.py` comment, test file, this Change Impact Log, and `ACTION_ITEMS.md` C110's own self-reference). No SQL content changed either time — only the filename/number. Confirmed via `ls backend/migrations | sort -V | tail` immediately before this second renumbering that 418 was the next genuinely free slot.
