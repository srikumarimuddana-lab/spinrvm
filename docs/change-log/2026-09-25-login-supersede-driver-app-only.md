...
- **Transition:** until a legacy rider-app device rotates (≤15 min) it still carries the shared id. If it logs out in that window, the old behaviour applies once.
- **Trust in `X-App-Platform`:** it is client-supplied. It decides only whether a login takes over the driver session and signs others out, which is a UX rule. Single-session enforcement (off in production) keeps its own server-side rule. The header never decides which session id a chain carries; ids are server-generated.
...
- [x] `spinr-security-auditor` review of the first version (blocker found and redesigned) and of this version (result in PR).
