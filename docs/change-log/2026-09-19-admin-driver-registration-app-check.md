...
- **The Firebase Console setup is deliberately paused, not just incomplete.** The Web app is registered (Step 1 done), but reCAPTCHA Enterprise requires the Firebase project to be on the Blaze (pay-as-you-go) plan — it's currently on the free Spark plan. Enabling Blaze means attaching billing to the Google Cloud project, and per-assessment cost applies past a free monthly tier. The user reviewed the actual pricing screen and chose to hold off rather than enable billing under time pressure — a business decision, not a technical blocker, and one this PR's fail-open design accommodates cleanly: nothing regresses by staying paused here indefinitely. This PR ships the code path fully wired but inert until Blaze is enabled, the Enterprise key is created, and the five env vars are set in Vercel — none of which block merging the code itself. Resuming later needs zero further code changes.
