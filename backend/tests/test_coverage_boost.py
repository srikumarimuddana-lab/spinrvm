...
class TestGetAiChatKey:
    """Tests for get_ai_chat_key / _extract_verified_user_id (ACTION_ITEMS.md AI1)."""
    ...
    def test_keys_by_user_id_claim(self):
        ...
        token = jwt.encode({"user_id": "rider_42"}, settings.JWT_SECRET, algorithm="HS256")
        req = MagicMock()
    ...
    def test_falls_back_to_sub_claim(self):
        ...
        token = jwt.encode({"sub": "driver_7"}, settings.JWT_SECRET, algorithm="HS256")
        req = MagicMock()
        req.headers = {"Authorization": f"Bearer {token}"}
    ...
    def test_two_ips_same_token_share_one_bucket(self):
        ...
        token = jwt.encode({"user_id": "rider_99"}, settings.JWT_SECRET, algorithm="HS256")
        req_a = MagicMock()
    ...
    def test_extraction_requires_a_valid_signature(self):
        """A token signed with the wrong secret must NOT select a user bucket
        (2026-09-20 review, C6): the bucket is shared, so an unverified decode
        let anyone forge a victim's user_id and exhaust their quota."""
        import jwt

        from backend.utils.rate_limiter import _extract_verified_user_id

        token = jwt.encode({"user_id": "victim"}, "not-the-real-jwt-secret", algorithm="HS256")
        req = MagicMock()
        req.headers = {"Authorization": f"Bearer {token}"}
        assert _extract_verified_user_id(req) is None
