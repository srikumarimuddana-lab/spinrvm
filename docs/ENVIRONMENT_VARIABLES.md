...
| `ALLOWED_ORIGINS` | **Required (prod)** | `"*"` | Comma-separated list of CORS origins. **`middleware.py` raises `RuntimeError` at startup if `"*"` is present and `ENV=production`.** Set to real domains before go-live (e.g. `https://admin-spinr.spinr.ca,https://track.spinr.ca`). Dev default is `http://localhost:3000,http://localhost:8081,http://localhost:19006`. | Hard-code per deployment |
...
| `ADMIN_REVOCATION_FAIL_CLOSED` | Optional | `false` | **SEC-A2-003.** What the admin per-JTI revocation denylist check (`admin:revoked:{jti}` in Redis, read in `_verify_admin_payload`, `backend/dependencies/__init__.py`) does when Redis errors. `false` = fail **open** (unchanged behaviour): the admin token is accepted, the error is logged at ERROR with a traceback and counted in `spinr_auth_revocation_check_error_total{outcome="fail_open"}`; the DB `token_version` check (logout-all) still runs. `true` = fail **closed**: the request gets a generic `503` and `spinr_auth_revocation_check_error_total{outcome="fail_closed"}` increments, so a single-session-revoked admin token cannot be used during a Redis outage — at the cost of **every admin request (HTTP and admin WebSocket) failing for the outage's duration**. Admin tokens only: rider/driver auth and `utils/session_revocation` tombstones never read it. Read from the process environment at startup, so changing it needs a restart, not a redeploy. Also covers the separate MFA-helper copy of the denylist check (`_require_staff_from_token` in `routes/admin/auth.py`, used by `/admin/auth/mfa/{status,enroll,confirm,disable}`), which returns the same `503` and increments the same counter. | Hard-code per deployment; leave `false` until the metric shows Redis error rates are acceptable |
