...
Incidental, forced by the pre-commit hook's whole-file lint/format check (not a scope expansion of the fix itself): `routes/drivers/profile.py:197` (unused loop variable `label`→`_label`) and `tests/test_drivers_shared_status_profile_coverage.py` (removed an unused `now_iso` in an unrelated test) — both pre-existing on `origin/main`, one-line, no logic change. `tests/test_drivers_extended.py` similarly needed a `ruff check --fix` (2 pre-existing import-sort findings) and a full-file `ruff format` (whitespace-only re-wrapping of pre-existing `@pytest.mark.parametrize` calls, ~44/-29 lines) to get its own commit through — confirmed no logic change by re-running the file's full 122-test suite before and after.
...
- **Found via CI, not local testing:** `test_drivers_extended.py::TestUpdateMyDriver::test_license_class_is_a_self_serve_vehicle_field` broke — a second `update_my_driver` test file this local sweep missed. Its `db_supabase.get_rows` mock returns the driver row unconditionally for **any** table argument (not just `"drivers"`), so the new `has_active_ride_obligation` call's `"rides"` query received that same non-empty row and was correctly read as "obligated" — the test's `is_online: False` assertion then hit a `KeyError` since the (correct, mid-trip-guard) code skipped the flip. Fixed by explicitly mocking `has_active_ride_obligation` to `False` in that one test, matching the pattern already used elsewhere. Confirmed via CI's `backend-test` job output, then reproduced and fixed locally; full file re-run afterward — 122 passed. All other CI failures on the same run were cross-checked against PR #5731's already-documented list of 23 pre-existing/order-dependent failures (13 identical-on-`main`, 10 order-dependent) plus the CR-2026-092 Postgres-credential issue affecting the whole real-Postgres CI tier (`test_atomic_driver_refund_holds.py`, the `dev/money/*.py` direct-pool tests) — none were a second new regression.
