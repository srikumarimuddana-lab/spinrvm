/**
 * app/reactivate-account.tsx (driver-app) — self-serve reactivation before
 * the 7-year deletion retention ceiling (Saskatchewan Transportation Act /
 * PIPEDA lawful-retention carve-out). Pins:
 ...
 *  - routes to /driver when the reactivated user is profile-complete, else
 *    /profile-setup
...
// Plain mutable-state hand-rolled mock, matching this app's own
// __mocks__/@shared/store/authStore.js convention (see app/index.tsx's own
// test file for why: driver-app's jest.config.js moduleNameMapper redirects
// every `@shared/*` import to `__mocks__/@shared/*`, so a zustand-based
// jest.mock factory here would just be shadowed by that file instead of
// actually intercepting the SUT's import).
...
describe('ReactivateAccountScreen (driver-app)', () => {
  ...
  it('reactivates, persists tokens, and routes to /driver for a profile-complete user', async () => {
    mockApiPost.mockResolvedValue({
      data: {
        token: 'access-1',
        refresh_token: 'refresh-1',
        expires_in: 900,
        user: { id: 'd1', profile_complete: true },
      },
    });
    const renderer = await renderScreen();
    await tapReactivate(renderer);
    ...
    expect(mockUseAuthStore.getState().user).toEqual({ id: 'd1', profile_complete: true });
    expect(mockShowToast).toHaveBeenCalledWith('success', 'Welcome back', 'Your account has been reactivated.');
  ...
  it('routes to /profile-setup when the reactivated user has an incomplete profile', async () => {
    mockApiPost.mockResolvedValue({
      data: { token: 'access-1', user: { id: 'd1', profile_complete: false, first_name: '', last_name: '', email: '' } },
    });
    const renderer = await renderScreen();
    await tapReactivate(renderer);
  ...
  it('shows the generic failure toast (not the 410 copy) for any other error', async () => {
