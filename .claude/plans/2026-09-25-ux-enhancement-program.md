...
| ID | Item | Surface | Gate | Verify |
|---|---|---|---|---|
| W4.1 `[x]` | Marker interpolation util (lerp position + bearing, snap on large jumps, off under Reduce Motion) for monitoring and the live-ride map | admin | none | unit tests on util; monitoring baseline unchanged (tiles stubbed) |
| W4.2 `[x]` | Same util on `/track`, plus brand tokens and a clear "arrived / trip ended" state. Adds no PII beyond today's driver name and plate | web | none | tests; `spinr-regulatory-compliance-checker` (PIPEDA) |
...
- 2026-09-26: W5.3 part a merged (#5902). Part b opened: a shared `EmptyState` (icon, title, hint, heading level) that copies the page-level empty block four pages already shared. Audit logs, staff and cloud messaging render the same DOM; promotions gains 16 px of padding. Part c, not started: service areas and vehicle types (the latter needs an action slot), the compact driver-panel variant (a size option), and the Rides and Drivers lists, whose visual baselines render empty states and need a re-capture `[H]`.
