...
`BookingProposalCard.tsx`'s inline (wallet) booking path called `createRide(paymentMethod, undefined, ...)`, hardcoding `corporateAccountId` to `undefined` regardless of the rider's corporate membership state. The standard `/ride-options` screen, by contrast, defaults `useCorporate = workModeEnabled` and passes `activeCompanyId` through, so `corporate_account_id`/`work_profile` reach the backend and `backend/routes/rides/booking.py`'s corporate policy check runs (as of 2026-08-24 this lives at `:922-957`, the pre-dispatch check, and `:1087-1141`, the `work_profile`+`corporate_account_id` block — the original `:717-721` citation has since drifted with unrelated edits and no longer points at this code). Card-payment proposals were unaffected because they already deep-link to `/ride-options` before booking (a card needs an explicit selection the chat card can't make) — only the wallet inline-booking path skipped it. Found by the 2026-07-28 AI guardrail audit; not a regression introduced by any recent change.
