...
6. **If there's no *active* automated visual/snapshot regression tooling for the surface you're touching, say so explicitly** rather than silently relying on "no visible diff" reasoning. rider-app and driver-app have none at all. admin-dashboard has a real, CI-wired Playwright visual-regression job (`e2e/visual-regression.spec.ts`) — but as of `ACTION_ITEMS.md` B38 it has zero committed baseline screenshots, so the job detects that and skips itself on every run; treat admin-dashboard the same as the other two surfaces (no active coverage) until baselines are seeded via `update-visual-baselines.yml` and B38 is closed. Flag the gap as a standing one (see `ACTION_ITEMS.md`) rather than re-discovering it every session.
...
**Background task safety** — the startup loops (live registry: `_WATCHDOG_LOOP_NAMES` in `core/lifespan.py`) run on every replica concurrently, unless the replica's `PROCESS_ROLE` says otherwise. `PROCESS_ROLE` ∈ `all` (default — HTTP + every loop, exactly the historical behaviour; local dev, tests and the Railway standby run on this), `web` (HTTP only; spawns no batch loop except `capacity_watchdog`, which measures *this* process), `worker` (the loops). It is set per-process, not app-wide — on Fly, `[env]` applies to every machine, so the role rides in each process group's command string. A production `web`/`worker` process refuses to boot without `REDIS_URL`, because the in-process dict fallback would give every replica its own leader lock and the split would silently do nothing.

26 loops hold Redis leader locks that **fail open** on Redis errors (see `docs/audit/2026-08-26-db-query-optimization-recommendations.md` §4.3). That is deliberate and must stay that way unless a specific loop proves otherwise: every one of them is *independently* replay-safe — atomic DB claim (dispatch, `stuck_ride_sweeper`, `push_retry`, `route_finalizer`, `orphaned_hold_reconciler`, `stale_p3_closer`), a `UNIQUE` claim row (`referral_payout`, `auto_payout`), a deterministic Stripe idempotency key (`corporate_autotopup`, `payment_retry`), or a `reminder_sent` flag. The lock sheds duplicate load; it is **not** the correctness guard. Making one fail closed would freeze legitimate work during a Redis blip while adding no safety — see `docs/change-log/2026-09-02-process-role-split.md` for the loop-by-loop verification. Any new loop must be replay-safe on its own; do not add a loop whose correctness depends on winning a lock.
