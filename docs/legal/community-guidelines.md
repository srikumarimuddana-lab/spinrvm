# Spinr Community Guidelines — Draft for Legal Review

> **What this is.** A standalone, published policy (not a signed contract) —
> meant to be reachable from both apps' Settings/Legal screens and from the
> website, and cited by name when Spinr suspends or deactivates an account.
> Today, ToS §7 (rider) and Part B §15 (driver) cover conduct in one short
> paragraph each; this document is what those clauses point to. Written in
> plain text with minimal formatting so it can be pasted into the same bare
> `<Text>` legal viewer used by `terms-of-service.md` and
> `privacy-policy.md` (`rider-app/app/legal.tsx`, `driver-app/app/legal.tsx`),
> or promoted to its own in-app screen — either works.
>
> **Why a standalone document, not just a ToS clause.** When Spinr deactivates
> an account, "you violated our Community Guidelines" only functions as a
> real answer to a dispute if the Guidelines are an actual, citable document
> a rider or driver could have read beforehand — not a single sentence buried
> in a Terms of Service nobody re-reads. App-store review teams and safety
> regulators also generally expect a marketplace platform to publish
> standalone conduct standards, separate from its liability/payment terms.
>
> **This is a draft, not legal advice** and has not been reviewed by counsel
...
1. Cross-reference against `docs/legal/non-discrimination-policy.md` and
   `docs/legal/driver-deactivation-appeals-policy.md` (companion documents
   drafted alongside this one) — all three are meant to be published
   together and should not contradict each other on what counts as a
   "serious violation."
2. Confirm with product/safety whether a temporary hold, as described here
