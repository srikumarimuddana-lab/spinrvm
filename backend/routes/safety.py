...
async def submit_safety_report(
    ...
    role = "driver" if current_user.get("is_driver") else "rider"
    return await record_safety_incident(body, current_user, role=role)


async def _notify_safety_team_logged(incident: dict) -> None:
...
async def record_safety_incident(body: SafetyReportRequest, current_user: dict, *, role: str) -> dict:
    """Persist a safety incident, then alert the safety team without blocking the reporter.

    `role` is the reporting surface, passed explicitly: `is_driver` is true for
    any account holding a driver row, so a dual-role person reporting from the
    rider app must still be recorded as a rider.
    """
    ...
    logger.info(
        f"[SAFETY] Incident {incident_id} reported by {role} {user_id} "
    ...
    # important part). Backgrounded because the email fan-out is sequential
    # per recipient (SES, then Resend at 10s) and could outlast the app's 15s
    # request timeout, showing "Submit Failed" for a saved report and inviting
    # a duplicate on retry.
    _spawn(_notify_safety_team_logged(incident))
...
async def upload_safety_report_photo(
    ...
    if len(existing) >= MAX_INCIDENT_PHOTOS:
        raise HTTPException(status_code=400, detail=f"At most {MAX_INCIDENT_PHOTOS} photos per report")
