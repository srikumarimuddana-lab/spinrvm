...
async def _run_full_dispatch(ride, driver, *, use_eta=False, eta_side_effect=None, max_offers=1):
    ...
    with (
        ...
        patch(
            "backend.routes.rides.matching._shared.dispatch.resolve_matching_config",
            AsyncMock(return_value=("nearest", 0, 10.0, max_offers, use_eta, 500)),
        ),
        patch(
            "backend.routes.rides.matching._deps.filter_and_rank_drivers",
            side_effect=lambda ride, drivers, *a, **kw: [(d, 1.0) for d in drivers],
...
async def test_max_offers_breaks_claim_loop():
    ...
    with (
        ...
        patch(
            "backend.routes.rides.matching._shared.dispatch.resolve_matching_config",
            AsyncMock(return_value=("nearest", 0, 10.0, 1, False, 500)),  # max_offers=1
        ),
        patch(
            "backend.routes.rides.matching._deps.filter_and_rank_drivers",
...
async def test_period_2_opened_for_each_claimed_driver_after_offer_insert():
    ...
    with (
        ...
        patch(
            "backend.routes.rides.matching._shared.dispatch.resolve_matching_config",
            AsyncMock(return_value=("nearest", 0, 10.0, 5, False, 500)),  # max_offers=5, both drivers claimed
        ),
        patch(
            "backend.routes.rides.matching._deps.filter_and_rank_drivers",
            side_effect=lambda ride, drivers, *a, **kw: [(d, float(i)) for i, d in enumerate(drivers)],
...
async def test_offer_timeout_handler_miss_threshold_lookup_exception_uses_default():
    ...
    with (
        ...
        # Since PR #5602 the release goes through this helper. _deps is a
        # MagicMock here, so without an AsyncMock the await raised
        # "object MagicMock can't be used in 'await' expression", which the
        # handler swallowed -- the release silently never happened.
    ...
    # Asserted on the helper, which now owns the release + period close.
    mock_deps.release_driver_and_close_period.assert_awaited_once()
    assert mock_deps.release_driver_and_close_period.await_args.args[0] == "drv-1"
