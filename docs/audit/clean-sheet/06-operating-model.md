...
|---|---|---|---|---|---|
| HIST-001 float money | Repository accepts untyped dicts; pre-commit check 6 warns only; CI rule has 7 exclusions and cannot see column types; three `rides` money columns are FLOAT8 | **Step 0: a write-site inventory** for every registered money column (repository helpers, the direct writes in `repositories/ride_repo.py` and `services/stripe_payout_sync_service.py`, and RPC argument builders); then a money-column registry + write guard that rejects `float`, applied to `ride_repo` and the payout sync first (flag `money_write_guard`); exit only at 100% coverage **and** zero rejections; pre-commit check 6 made blocking or deleted; extend the CI rule to `earnings.py`, `features.py`, `referral_payout.py` | Not built; 5 new instances found this wave (MONEY-006/-008, MONEY-001) | N19, X5 | BE-PAY |
| HIST-002 / HIST-015 forks | "Surgical change" rule; parity guard diffs props only; pre-commit check 11 warns | **Rule change** (E-F14): reconcile or contract-test a fork before the second fix; a PR-level CI parity check with a PR-body override (the local hook stays warn-only — a local hook is not a gate); a per-fork contract test as the definition of done; every `known-forks.md` row gets a reconciliation date; register the admin track-page marker | Registry exists (4 rows, 1 copy unregistered); guard warn-only | X16, E-F14 | MOBILE + REPO-ADMIN |
...
| HIST-006 migrations | Hand numbering across parallel sessions; manual apply; no owner | Named owner (E-F9); nightly `--status` alert; then migrate-on-deploy gated on CI evidence (the Fly deploy-gate pattern of 2026-09-23), `auto` mode for expand-only migrations and `approve` for contract steps; scheduled read-only advisor diff against repo intent. (Timestamp prefixes dropped: CHECK B and the nightly duplicate sweep already catch new collisions — 08-hostile-review U-13.) | Collision check (CHECK B) exists and blocks; apply is manual | X9 | DB-OWNER |
...
| **Do not add blocking gates to a gate system that already cries wolf.** Fix the false-red summary jobs (QUAL-001) and publish the branch-protection answer (C21) before promoting any new gate to blocking; have at most two new gates in warn mode at a time | RR-01, RR-03, all §1 gates | sequencing rule in `ROADMAP.md` X20 | CI |
...
5. **Review coverage for the two unowned surfaces** by widening existing agents' charters, not adding new agents (more agents is more config for the same two people to maintain): `shared/` logic and Maps & Routing (CARTO-002, CARTO-003).
6. **A work-in-progress limit.** The visible history shows about two human authors plus agent sessions. Run at most two roadmap tracks at once, and collapse the owner roles into a few named human hats (for example founder, backend lead, mobile lead) with agents as reviewers, not owners (08-hostile-review §1.8). Do not ship any queue that needs a human response time (money approvals, stranded-rider requests) until an on-call rota exists.
