...
Recommendations: 2 Critical/High, 3 Medium, 2 Low (Section 8).
...
5. The job row is updated to `completed` with a `storage_path` and `expires_at`; the initial response includes a **7-day signed URL**. After that, `data_transfer_jobs.py`'s `/download` endpoint (now super_admin-gated — see the 2026-07-28 access-control fix) can regenerate a fresh 1-hour signed URL as long as the job hasn't been purged.
...
**R-001 — Access control is broader than the data's sensitivity warrants**
- Description: The export/import/search/SGI-forms routes are gated on the `bulk_operations` admin module flag — a role-assignable permission, not `super_admin`. Any admin with that flag (which may include ops/support staff granted it for unrelated bulk-import work) can export full unredacted PII for up to 100 people at a time, including government ID numbers and exact location history, with no per-export justification captured.
- Likelihood: 3 (Possible) — module flags tend to accumulate broadly over time as convenient rather than being tightly scoped.
- Impact: 4 (Major) — a single export action can move a large batch of highly sensitive PII.
- Risk Rating: **High** (3×4=12)
- PIPEDA principle at risk: 4.7 (Safeguards) — access should be commensurate with sensitivity.
- Mitigation: See Recommendation R-A below.
- Residual Risk (after mitigation): Low-Medium.
- Owner: Whoever administers admin role/module assignments at Spinr.
...
- Description: Once an admin has the signed URL and downloads the ZIP, the data is now a file on that admin's local machine, entirely outside Spinr's Supabase access-control perimeter, indefinitely.
...
**[HIGH] R-A — Tighten export/import/search/SGI-forms access beyond the generic `bulk_operations` flag, or add a stated-purpose + secondary-approval step**
Two viable options, either is a real mitigation for R-001: (1) split a new, narrower module flag (e.g. `data_transfer_pii_export`) specifically for this feature so it isn't bundled with unrelated bulk-import permissions, letting admin-role assignment be deliberate rather than incidental; or (2) require `super_admin` outright, matching the tightening already applied to the job-history/download endpoints in this session's work. Recommend option (1) as the better long-term fit — it preserves delegation to trusted non-super-admin operators (e.g. a dedicated data/migrations engineer) while still making the grant explicit and auditable, rather than forcing everything through `super_admin`. Owner: whoever owns RBAC design for the admin dashboard. Success criterion: the export/import/search/SGI routes require a flag no other unrelated feature also grants.
