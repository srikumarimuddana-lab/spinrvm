...
## Step 5 — relink drivers whose live account already exists

Step 4 assumes the driver re-onboards. Some will not need to: a driver who
already completed Connect onboarding **under the live key** has a real
`acct_…` sitting in the Stripe dashboard while `drivers.stripe_account_id` is
NULL (retired by the cutover, or never written). Nothing finds these on its
own — every refresh path follows the column and refuses to guess, so the
driver looks like "no Stripe account" no matter how many times you refresh.

Relinking is a four-step loop in **Admin → Bulk Operations**, kind `drivers`:

1. **Find matches by email.** Read-only. Lists unlinked drivers, lists live
   connected accounts, and pairs them on **exact, case-insensitive email
   equality, one-to-one only**. Never fuzzy (`a@x.com` ≠ `a+stripe@x.com`),
   never an account already on another driver's row. One email on two drivers,
   or two accounts, is reported as `ambiguous` and matched by nobody. It
   downloads `drivers_mapping_by_email.csv` — two columns,
   `stripe_account_id,phone`. It writes nothing.
2. **Read the report before uploading.** `ambiguous` and `matches_without_phone`
   are *not* in the CSV and need the per-driver update flow instead — a driver
   row with no phone has nothing for the import to match on.
3. **Upload that CSV → Validate.** The import re-matches locally on phone, then
   retrieves every `acct_…` from live Stripe. Per-row outcomes:
   `already_mapped` (warning, skipped — already correct), `needs_update`
   (driver holds a *different* account; never bulk-written, confirm per driver),
   `no_match`, `duplicate_in_csv`, `conflict_existing`. Errors do not block the
   healthy rows.
4. **Commit → then Refresh Stripe KYC.** Commit only fills rows where
   `stripe_account_id` is **NULL** — it cannot overwrite a live payout
   destination. The KYC sync then mirrors real Stripe state
   (`payouts_enabled`, `details_submitted`, requirements) onto each driver.

Cap: 200 rows per CSV. Both endpoints are `super_admin` only and audited.

Phone spelling is handled on both sides — discovery echoes the phone exactly
as stored, and the import looks up the normalized *and* verbatim spellings —
so a CSV generated by discovery matches the row it came from regardless of
whether `drivers.phone` is E.164. Hand-written CSVs may use either form.
