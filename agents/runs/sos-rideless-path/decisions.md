...
2. `backend/tests/test_sos_rideless.py`'s `idempotency_key = "abcdefgh12345678"` test fixture, flagged by Gitleaks' `generic-api-key` rule. Verified directly (read the surrounding test) that this is a hardcoded string used purely to test idempotency-key deduplication, not a real credential — renamed to `"test-retry-idem-001"` rather than adding a `.gitleaksignore` suppression, since a suppression keyed to this commit's hash would become stale the moment the branch is rebased/squashed, while a self-evidently-fake string fixes the false positive permanently and improves readability.
