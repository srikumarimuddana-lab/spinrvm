...
- **Nielsen heuristic gaps** — load `.claude/context/ux-ui-principles-spinr.md` and judge holistically (beyond `spinr-design-consistency-reviewer`'s mechanical version of the same checklist): does this screen give the user real control/freedom through a flow, help recognition over recall (especially driver-app's glanceable context), and offer efficiency paths without ever making a driver-app "efficiency" feature feel mandatory? Cite the doc's guidance rather than re-deriving the principle from scratch.
...
4. **Data visualization consistency** — for any chart/analytics/dashboard page, load the `dataviz` skill and judge against it: consistent categorical/sequential palette (not ad-hoc per-chart colors), legible in both themes, real empty/loading states on the chart itself (not just the page), and a stat-tile/KPI treatment consistent with sibling pages. This is a real gap today — no admin-dashboard chart page has been checked against this skill before now; don't assume prior charts are compliant just because they predate this check.
