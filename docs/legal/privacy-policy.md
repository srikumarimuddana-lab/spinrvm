...
1. **Data residency** — the Canadian-hosting statement in section 3 needs `reports/legal/supabase-region-attestation-checklist.md`'s open steps closed (region formally verified, `SUPABASE_REGION=ca-central-1` confirmed set in production) before it's a fully attested fact rather than an architectural intent.
