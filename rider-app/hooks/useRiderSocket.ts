  ...
  const handleMessage = useCallback((data: any) => {
    ...
    switch (data.type) {
      ...
      case 'ride_cancelled': {
        ...
        // A late cancel for a previous ride must not clear the ride on screen.
        const rideState = useRideStore.getState();
        const cancelledRideId = typeof data.ride_id === 'string' ? data.ride_id : undefined;
        if (!shouldLeaveScreenForRideCancelled(
          cancelledRideId,
