...
- [ ] **Not yet verified**: an actual live GitHub Actions run of this new job graph. `actionlint`/`yamllint` were not available in this environment to check beyond `yaml.safe_load`-level syntax (no invalid-expression or invalid-action-input linting was possible locally). This workflow triggers on this exact PR's next push (`pull_request: synchronize`), so the real test happens immediately after this commit is pushed — watching that run's actual result is the next step, not assumed to already be green by this log's existence.
