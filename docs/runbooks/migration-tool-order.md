...
| 16 | Pre-Launch Legacy Data Flagging | `/dashboard/bulk-operations` | Needs #1/#2's source markers on drivers, #3's `rider_csv_import` marker on riders, **and** #11's written rides — run last so the full population exists before deciding what's dormant/pre-launch |
...
**Extended 2026-09-10**: #16 (Pre-Launch Legacy Data Flagging) now also flags dormant legacy riders (`users` table, zero rides ever), not just drivers/rides — closing a gap found in `docs/change-log/2026-09-10-a34-pre-launch-data-contamination-check.md`. See `docs/change-log/2026-09-10-pre-launch-flag-riders.md`.
