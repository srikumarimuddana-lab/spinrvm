...
import { getDriverStats, getDriverDocuments, reviewDocument } from "@/lib/api";
...
import {
    ...
    CheckCircle, XCircle, FileText, Phone, Mail, CalendarRange, Filter,
} from "lucide-react";
...
export default function DriversPage() {
    ...
    const loadData = useCallback(() => {
        ...
        getDriverStats(params)
            .then((res) => {
                ...
                setServiceAreas(res.service_areas || []);
            })
            .catch(() => {})
    ...
    // Load selected driver's documents
    useEffect(() => {
        if (!selected?.id) {
            setDriverDocs([]);
            return;
        ...
        try {
            ...
            if (status === "approved") {
                ...
                if (input && input.trim()) {
                    ...
                    if (isNaN(d.getTime())) { alert("Invalid date. Use YYYY-MM-DD."); setDocBusy(null); return; }
    ...
    const selectedAreaName = serviceAreaId
        ...
        <div className="space-y-5">
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        {drivers.length} drivers {serviceAreaId ? `in ${selectedAreaName}` : "overall"}
                    </p>
                </div>
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <Input type="date" value={startDate} onChange={e => setStartDate(e.target.value)}
                            className="h-9 w-[140px] text-xs" placeholder="Start" />
                        <span className="text-xs text-muted-foreground">to</span>
                        <Input type="date" value={endDate} onChange={e => setEndDate(e.target.value)}
                            className="h-9 w-[140px] text-xs" placeholder="End" />
                    </div>

                    {/* Clear Filters */}
                    {(serviceAreaId || startDate || endDate) && (
                        <button onClick={() => { setServiceAreaId(""); setStartDate(""); setEndDate(""); }}
                            className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground px-2 py-1.5 rounded-lg hover:bg-muted transition">
                    ...
                    <button onClick={handleExport} disabled={filtered.length === 0}
                        className="flex items-center gap-1.5 text-sm font-medium text-muted-foreground hover:text-foreground transition px-3 py-1.5 rounded-lg hover:bg-muted disabled:opacity-50">
            ...
            {!serviceAreaId && (
                <AreaStatsTable
                    ...
                    loading={loading}
                    onAreaClick={(areaId) => setServiceAreaId(areaId)}
                />
            )}

            {/* Drivers List + Detail Split */}
            <div className="flex gap-4 min-h-[500px]">
                ...
                {/* Right: Driver Detail */}
                {selected && (
                    <div className="w-1/2 lg:w-2/5 bg-card border rounded-2xl overflow-y-auto max-h-[700px]">
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            <Section title="Contact">
                                ...
                                <InfoRow icon={MapPin} label="Service Area" value={
                                    serviceAreas.find(a => a.id === selected.service_area_id)?.name || selected.service_area_id?.slice(0, 8) || "Not set"
                                } />
                            </Section>
                            ...
                            <Section title="Uploaded Documents">
                                {docsLoading ? (
                                    ...
                                    <div className="space-y-2">
                                        {driverDocs.filter(d => d.status !== "superseded").map(d => {
                                            const exp = d.expiry_date || d.expires_at;
                                            const expired = exp && new Date(exp) < new Date();
                                            const statusColor = d.status === "approved" && !expired ? "text-emerald-600"
                                                ...
                                                <div key={d.id} className="bg-background rounded-lg p-2.5 border">
                                                    <div className="flex items-start justify-between gap-2">
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            {exp && <p className="text-[11px] text-muted-foreground">Expires: {new Date(exp).toLocaleDateString("en-CA")}</p>}
                                                            {d.document_url && (
                                                                <a href={d.document_url} target="_blank" rel="noreferrer" className="text-[11px] text-primary underline">View file</a>
                                                            )}
                                                            {d.rejection_reason && <p className="text-[11px] text-red-500 mt-1">Reason: {d.rejection_reason}</p>}
                                                        </div>
                                                        <div className="flex flex-col gap-1 shrink-0">
                                                            <button onClick={() => handleReviewDoc(d.id, "approved")} disabled={docBusy === d.id}
                                                                className="px-2 py-1 rounded bg-emerald-500 text-white text-[11px] font-semibold hover:bg-emerald-600 transition disabled:opacity-50">Approve</button>
                                                            <button onClick={() => handleReviewDoc(d.id, "rejected")} disabled={docBusy === d.id}
                                                                className="px-2 py-1 rounded bg-red-500 text-white text-[11px] font-semibold hover:bg-red-600 transition disabled:opacity-50">Reject</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            );
