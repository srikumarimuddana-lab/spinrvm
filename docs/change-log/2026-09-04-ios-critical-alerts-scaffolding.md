...
**Correction (added after a self-review pass on this branch):** an earlier draft of this entry described enabling the flag as needing only the entitlement plus a shipped build. That was wrong and is corrected here rather than left to mislead whoever eventually flips it. There is a **third** prerequisite: the driver app must actually request `criticalAlert` authorization at runtime, and it does not. `notifeeService.ts:156-161` calls `requestPermission({alert, sound, badge, provisional})` with no `criticalAlert`, so `UNAuthorizationOptions.criticalAlert` is never granted and iOS would not honour a critical alert even with the entitlement installed. The comment directly above that call (`notifeeService.ts:153-155`) claims critical is "attempted… falls back gracefully if denied" — it is not attempted at all; that pre-existing, inaccurate comment is very likely what caused the original mis-scoping. The client fix is deliberately **not** made here: it is untestable from this environment (no simulator, and `yarn install` does not work in this session — see §10) and is dead code until the entitlement exists. All three prerequisites are now enumerated at the flag's definition in `backend/core/config.py`, which is where someone about to flip it will actually look.
