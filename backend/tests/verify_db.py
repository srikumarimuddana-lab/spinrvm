...
import backend.db_supabase as db_supabase  # noqa: E402
...
class TestDBWrapper(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_find_nearby_drivers(self):
        with patch("backend.db_supabase.supabase") as mock_supabase:
            ...
            mock_supabase.rpc.assert_called_with(
                "find_nearby_drivers", {"lat": 52.1, "lng": -106.6, "radius_meters": 5000}
            )
    ...
    async def test_update_driver_location(self):
        with patch("backend.db_supabase.supabase") as mock_supabase:
            ...
            await db.drivers.update_one({"id": "d1"}, {"$set": {"lat": 52.2, "lng": -106.7}})

            mock_supabase.rpc.assert_called_with(
                "update_driver_location", {"driver_id": "d1", "lat": 52.2, "lng": -106.7}
            )
    ...
    async def test_claim_driver_atomic(self):
        with patch("backend.db_supabase.supabase") as mock_supabase:
            ...
            # Test logic
            res = await db.drivers.update_one({"id": "d1", "is_available": True}, {"$set": {"is_available": False}})
