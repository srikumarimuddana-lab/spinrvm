...
| Sentry | Account-level (`.mcp.json` also declares it) | URL path scoping: `https://mcp.sentry.dev/mcp/{org}/{project}` (Sentry's own docs recommend project-level scoping) | **Still needs narrowing.** Sentry access comes via the Fly.io subscription rather than a standalone Sentry account — that only affects billing/plan tier, not the scoping mechanism, which is a Sentry org/project concept independent of who pays. `.mcp.json` still points at the unscoped base URL. Waiting on the org slug + project slug (visible in the Sentry dashboard URL, or wherever Sentry issues are viewed via the Fly integration) to apply the scoped URL. | 2026-09-07 |
