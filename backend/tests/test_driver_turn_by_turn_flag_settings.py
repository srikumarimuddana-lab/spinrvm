...
super-admin gate, backed by migration 416's column (see
...
def test_migration_416_adds_the_column_with_false_default():
    ...
    sql = (
        Path(__file__).resolve().parents[1] / "migrations" / "416_settings_driver_turn_by_turn_enabled.sql"
    ).read_text(encoding="utf-8")
    match = re.search(rf"{_FLAG}\s+BOOLEAN NOT NULL DEFAULT (TRUE|FALSE)", sql, re.IGNORECASE)
    assert match, f"{_FLAG} not declared with an explicit boolean default in migration 416"
