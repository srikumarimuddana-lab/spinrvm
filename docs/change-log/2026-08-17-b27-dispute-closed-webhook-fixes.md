...
| `backend/tests/test_routes_webhooks_coverage.py` | Existing 3 dispute-closed tests updated (dispute payloads now include `id`, rider-lookup mocked); 6 new tests: `warning_closed` regression pin, dispute-id-keyed lookup assertion, PI-less-close-updates-only-its-own-row, balance-transactions-recorded, no-balance-transactions-skips-ledger, plus a new `TestStripeWebhookDisputeUpdated` class (2 tests) | Regression coverage for all 3 fixes + the new handler |
...
- [x] `spinr-money-auditor` review requested for this diff before PR creation (Codex auto-review is off per CLAUDE.md C7/C9).
