...
> **No automated PR review runs on this repo.** The Codex integration
> (`chatgpt-codex-connector`) was removed — it went silent on 2026-07-30 after
> reviewing 183 PRs, the cause was never diagnosed as fixable from this side
> (see `ACTION_ITEMS.md` C9, closed), and the decision was made to stop
> depending on it rather than keep waiting. The Claude agent audit
> (`claude-review.yml`) remains off by design — `ANTHROPIC_API_KEY`
> deliberately unset on cost grounds, see C7.

A PR touching money, auth, migrations, dispatch, or safety needs a **manual**
review pass before merge — a human, or the relevant `spinr-security-auditor` /
`spinr-money-auditor` / `spinr-migration-reviewer` subagent invoked via the
Agent tool. When subscribed to a PR (or asked to look at one), **do not chase
CI checks** — skip `yarn audit` / `npm audit` / lint / deploy status unless
the user explicitly asks. Pre-existing dependency-audit failures on surfaces a
PR doesn't touch are not this PR's job.
...
**Background task safety** — the 41 startup loops run on every replica concurrently (27 hold best-effort Redis leader locks that fail open on Redis errors — see `docs/audit/2026-08-26-db-query-optimization-recommendations.md` §4.3). Dispatch uses an atomic DB claim; others use `reminder_sent` flags or idempotency keys. Any new loop must be replay-safe.
