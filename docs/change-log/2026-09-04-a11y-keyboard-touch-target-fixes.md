...
**Round 2 — `spinr-accessibility-reviewer` found a real regression, fixed:** the subagent review (run after the above, before merge, per this PR's own checklist) caught that the 12 new `onKeyDown` handlers bubble from any focused descendant — including the nested "Actions" cell buttons (Delete/Edit/Cancel/a Switch toggle) that already guard their `onClick` against bubbling to the row but had no equivalent keydown guard. A keyboard user tabbing to a nested button and pressing Enter fired both that button's own action *and* the row's action. Worst case, confirmed directly: `venues/page.tsx`'s "Delete" button called `remove(v)` on Enter, and the bubbled keydown then also called `startEdit(v)` on the just-deleted venue. Fixed by adding `if (e.target !== e.currentTarget) return;` as the first line of all 12 `onKeyDown` handlers — the row only acts when the row itself (not a descendant) is the focused element. Applied uniformly rather than patching each nested control individually. The pre-existing reference example (`drivers/page.tsx:1047`) has the same latent gap but is currently harmless (its nested button calls the same function the row does) — left untouched, out of this PR's stated scope, flagged for a future round.

The reviewer also flagged `manage-cards.tsx`'s delete-button hitSlop encroaching into the 10pt gap toward its neighboring "Set Default"/"Use & Pay" button (8pt hitSlop into a 10pt gap ≈ 2pt true clearance) — reduced that button's left-side hitSlop to 2pt (top/bottom/right unchanged at 8pt) so the two buttons' tap-catch areas no longer crowd each other.
...
- [x] Reviewed against CLAUDE.md conventions: task decomposition (7 commits, ≤3 files each admin-dashboard batch), surgical/additive-only changes, no PII/logging/auth/money surface touched.
- [x] `spinr-accessibility-reviewer` subagent run against the full diff before merge — found one real blocker (keyboard event-bubbling double-activation, §3 "Round 2") and one real warning (hitSlop/adjacent-button crowding, same section), both fixed and re-verified (tsc/eslint/full suites all re-run clean after the fix, same results as the first pass).
...
- No on-device or emulator testing for the mobile-app hitSlop changes, and no screen-reader (VoiceOver/TalkBack) testing for the admin-dashboard keyboard fixes — none available in this environment. (`spinr-accessibility-reviewer` *was* run against this diff before merge — see §9 — but its findings are still static-analysis-level reasoning, not a live device/screen-reader pass.)
- The reviewer also noted a residual, pre-existing gap this PR doesn't fix: a bare `<tr>` with `tabIndex`/`onKeyDown` has no accessible interactive role (`role="row"`, not `role="button"`), so some screen readers won't announce these rows as activatable even though they're now keyboard-reachable. This matches the codebase's existing convention (including the original `drivers/page.tsx:1047` reference example) rather than being introduced by this PR — flagged as a follow-up, not fixed here.
