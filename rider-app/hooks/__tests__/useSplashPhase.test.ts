 ...
 * forever.
 *
 * Code under test: rider-app/hooks/useSplashPhase.ts
 */
...
describe('useSplashPhase', () => {
  ...
  it('exits promptly when the app opens straight onto a deep link', () => {
    const { result } = renderHook(() =>
      useSplashPhase({ navReady: true, pathname: '/ride-in-progress' }),
    );
    act(() => {
      jest.advanceTimersByTime(SPLASH_EXIT_SETTLE_MS + 1);
    });
