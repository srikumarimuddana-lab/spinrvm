...
| 12 | Admin actions with no audit trail | **STILL-OPEN, one instance fixed** | same ~12 baseline endpoints + `driver_appeals.py` resolve action unchanged; **FIXED 2026-08-19**: `POST /admin/redis/flush-prefix` now writes an audit row (see N11 above) — was the highest-severity instance (destructive prod cache wipe), but the broader ~12-endpoint sweep is still open |
