...
| `drivers.total_rides` recount | Was 2 round trips per driver (128 calls for 64 drivers, 20-40s); an HTTP timeout mid-loop leaves counters half-updated with no signal which | Migration 269 set-based RPC — one statement, all-or-nothing; Python loop kept as fallback for databases without 269 |
...
- `recount_driver_total_rides` (migration 269) is a **new** function — no existing caller. `recount_drivers()` is called only by `commit_plan` in this service; grepped for other callers, none. The Python fallback is byte-identical to the previous behaviour, so a database without 269 behaves exactly as before this change.
...
| `backend/migrations/269_recount_driver_total_rides_fn.sql` | New: set-based `recount_driver_total_rides(text[])` RPC | Replaces a 2-round-trips-per-driver loop; one statement that cannot leave counters half-updated when an HTTP caller times out |
...
- [x] **Migration 269 linted**: `backend/scripts/check_migration.py` — naming, sequence (269 follows 268), rollback comment, dangerous-ops all pass.
...
- **Migration 269's RPC has never executed against Postgres.** Its SQL is unit-tested only through a Python fake that emulates the intended semantics — which proves the *caller* is correct, not that the function body is. The `WITH counts … UPDATE … FROM counts` statement should be applied and its return value checked against a manual `COUNT(*)` on first use. If it fails to apply, the Python fallback keeps the importer working, so this is not a blocker for merging — but the RPC is unproven until it runs.
