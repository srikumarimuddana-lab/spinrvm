...
- [x] **Automated tests run (final, after the §13 blocker fixes).** Backend: **126/126** across `test_webhook_payment_failed_guard.py` (24), `test_live_breadcrumbs.py` (8), `test_live_route.py` (8), `test_db_run_sync_cancellation_releases_probe.py` (3), `test_server_coverage.py`, `test_driver_sin_collection.py`. driver-app: **372/373** across 36 suites (`hooks/__tests__`, `__tests__/hooks`, `lib/androidAuto/__tests__` 234, the new real-SQLite suite 6) — the single failure is the pre-existing CRLF artifact documented below.
- ⚠️ **Do not read that 126 as coverage of the `/live-route` fix.** `backend/tests/test_live_route.py:64` patches `utils.live_breadcrumbs.build_breadcrumb_trail` with a stub, so its 8 tests exercise the route handler's wiring and give **zero real coverage** of the function this batch actually changed (CLAUDE.md pre-merge gate 1). The real coverage for change #3 is `test_live_breadcrumbs.py`'s 8 tests, which call `get_gap_filled_breadcrumbs` for real — including the haversine fallback branch the new connector cap falls through to. Corrected here rather than left implied by the pass count.
