...
Add the exact path `/api/v1/ai/public-chat` to `_APP_CHECK_EXEMPT_PREFIXES` and `_CSRF_EXEMPT_EXACT`. No other `/api/v1/ai/*` route is exempted.
...
|---|---|---|
| backend/core/middleware.py | Added `/api/v1/ai/public-chat` to `_APP_CHECK_EXEMPT_PREFIXES` and `_CSRF_EXEMPT_EXACT` with rationale comments | Browser surface cannot attach App Check, and an anonymous visitor has no CSRF cookie |
| backend/tests/test_appcheck_ai_public_chat_exempt.py | New test | Public chat reaches its handler through the live App Check + CSRF ordering with a browser `Origin`; signed-in AI routes remain enforced; exemptions are exact and match the real served route |
...
```
# After
POST /api/v1/ai/public-chat  -> handler even with browser Origin and no CSRF/App Check tokens (403/503 if kill switches off, 429 if rate limited, 200 otherwise)
...
- [x] Regression test mounts CSRF outside App Check in the same relative order as `init_middleware` and sends a browser `Origin` without CSRF or App Check tokens.
