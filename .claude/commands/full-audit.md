...
## 2 · Dispatch all 20 reviewer agents, in parallel, unconditionally
...
Present all 20 verbatim, grouped by the four tables above, then the code-quality pass, then one rollup:

```
SPINR FULL-FLEET AUDIT — <scope>
==================================
Files: X changed | +Y -Z lines
Agents dispatched: 20/20
...
The rollup is the worst verdict across all 20 plus the inline pass. Never soften any single agent's verdict.
