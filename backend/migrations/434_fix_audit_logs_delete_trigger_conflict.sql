-- 434_fix_audit_logs_delete_trigger_conflict.sql
-- ACTION_ITEMS.md C112: migration 57's audit_logs_no_mutate trigger (BEFORE
-- UPDATE OR DELETE, unconditional) conflicts with migration 56's
-- audit_logs_no_delete trigger (BEFORE DELETE, flag-gated on the session GUC
-- spinr.audit_logs.allow_delete so purge_pii_retention()'s Step G can still
-- perform the sanctioned 7-year regulatory retention delete). Postgres fires
-- every applicable BEFORE ROW trigger for one statement: on DELETE,
-- audit_logs_no_delete (56) fires first and, when the flag is set, lets the
-- delete through -- but audit_logs_no_mutate (57) then fires next and aborts
-- unconditionally anyway, with no knowledge of the flag at all. This breaks
-- the one sanctioned delete path Step G exists to perform.
--
-- FIX (option (b) from C112's writeup, chosen over making 57's trigger
-- flag-aware on DELETE too): narrow 57's audit_logs_no_mutate trigger to
-- UPDATE only, leaving 56's audit_logs_no_delete as the sole DELETE guard.
-- Simpler than duplicating the flag-check logic across two triggers, and
-- consistent with the precedent migration 317's own
-- check_disabled_guard_triggers() comment already accepts: 57's trigger is
-- already redundant with 51's audit_logs_no_update on the UPDATE side, so
-- narrowing its scope down (rather than widening it) matches how this
-- codebase already treats that overlap. 51 and 57 both still independently
-- guard UPDATE after this migration -- unchanged, this migration touches the
-- DELETE clause only.
--
-- MANDATORY companion fix, not optional: migration 335 (the current live
-- purge_pii_retention() body) never carried forward migration 56's Step G
-- shape (PERFORM set_config(...) before the DELETE, wrapped in
-- BEGIN/EXCEPTION WHEN OTHERS/RAISE, same pattern as Step H's
-- financial_events delete and Step M's compliance_export_events delete in
-- the same function) -- it is a bare IF/ELSE with no set_config call at all,
-- so today's DELETE branch is simply dead code (falls through to the ELSE's
-- SELECT COUNT every time) and nothing crashes. Restoring the flag-set call
-- WITHOUT restoring the missing exception handler, or without this
-- migration's trigger fix, would convert today's silent no-op into an
-- unhandled-exception rollback of every retention step in the same function
-- call (Steps A-N all run inside one function invocation) -- so both halves
-- ship together in this one migration.
--
-- Verified against the actual, current live function body (335) and trigger
-- definition (57) by direct file read, not inferred -- see ACTION_ITEMS.md
-- C112's 2026-09-14 correction for the full trace of which version is live.
--
-- Rollback:
...
--   -- then re-apply migration 335's purge_pii_retention() definition verbatim
--   -- to restore Step G's pre-fix (dead-code) shape. Rollback of the function
--   -- does not undo any already-purged data (Step G's DELETE only removes
--   -- rows already past the 7-year retention ceiling).
--
...
-- same intentional re-fork pattern as migrations 296/321/323/324/335.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Narrow migration 57's audit_logs_no_mutate trigger to UPDATE only.
--    Same function (_audit_logs_immutable), same trigger name -- only the
--    firing event changes. _audit_logs_immutable()'s error message ("cannot
--    be modified or deleted") stays accurate: an unflagged DELETE is still
--    fully blocked, just by 56's audit_logs_no_delete trigger instead.
-- ─────────────────────────────────────────────────────────────────────────────
DROP TRIGGER IF EXISTS audit_logs_no_mutate ON audit_logs;
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Re-fork purge_pii_retention VERBATIM from migration 335 (current live
--    version) and fix ONLY Step G: add the PERFORM set_config(...) call and
--    wrap the DELETE in the same BEGIN/EXCEPTION WHEN OTHERS/RAISE shape
--    every other gated delete step in this function uses (compare Step H's
--    financial_events delete, Step M's compliance_export_events delete).
