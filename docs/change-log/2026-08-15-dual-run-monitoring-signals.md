...
- The `auto_payout` background loop and admin-initiated payout paths were NOT instrumented (only the two driver-initiated request paths) — if weekly auto-payout (migration 314 era) transfers to legacy drivers, those settle uncounted. Scoped follow-up if launch-week dashboards need it.
