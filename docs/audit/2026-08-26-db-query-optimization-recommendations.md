...
| `routes/drivers/earnings.py:57` and `:91` | `GET /drivers/balance` | Two 10,000-row `rides` scans. **Correction (2026-08-27): these are not the same query** — `:57` applies `EXCLUDE_LEGACY_RIDES` (money), `:91` deliberately omits it (activity count), documented as fix A31 at `:80-90`. Collapsing them would reintroduce "driver with only legacy rides shows `total_rides = 0`". Only `:91` is convertible, and only to a **legacy-inclusive** count | Partly — `count_documents` at `:91` only, keeping the filters distinct |
...
| 13 | **Replace `len(get_rows(...))` with `count_documents`** — **correction (2026-08-27): only `drivers/earnings.py:91` actually qualifies.** `admin/faqs.py:205/210/215` consumes the rows in a push fan-out loop, not a count; `rides/booking.py:523` feeds the overlap check at `:554-575`, so a count would *add* a round-trip. See the §4.1 correction above for why `:91`'s conversion must stay legacy-inclusive | Removes one bulk transfer that produces a single integer | Low. Note `db.py:94`'s shim is itself wrong above 1,000 | S |
